...
void spawner_task(const void *args, size_t arglen, const void *userdata, size_t userlen,
                  Processor p)
{
  ...
  for(size_t i = 0; i < s_args.config.tasks_per_proc; i++) {
    ...
    if(s_args.config.max_exec_time > s_args.config.min_exec_time)
      w_args.exec_time +=
          PRNG::rand_int(PKEY_DELAY, s_args.index, i,
                         (s_args.config.max_exec_time - s_args.config.min_exec_time));
    // work tasks get a higher priority to get ahead of the reaping task
    Event e = target.spawn(WORK_TASK, &w_args, sizeof(w_args), ProfilingRequestSet(),
                           Event::NO_EVENT, 1);
    ...
    reap_events(s_args.cq, in_flight, s_args.config.max_in_flight - 1,
                s_args.config.max_to_pop);
  }

  // reap events until we have none in flight
  reap_events(s_args.cq, in_flight, 0 /*max_in_flight*/, s_args.config.max_to_pop);
}

void test_cq(const TestConfig &config, const std::vector<Processor> &procs,
             size_t cq_size)
{
                    ...
                    Processor p)
{
  ...
  log_app.print() << "compqueue test: " << procs.size() << " procs, "
                  << config.tasks_per_proc << " tasks/proc, " << config.max_in_flight
                  << " in flight, " << config.max_to_pop << " max pop";
...
int main(int argc, const char **argv)
{
  ...
  Processor::register_task_by_kind(p.kind(), false /*!global*/, TOP_LEVEL_TASK,
      ...
      .external_wait();
  Processor::register_task_by_kind(p.kind(), false /*!global*/, SPAWNER_TASK,
                                   CodeDescriptor(spawner_task), ProfilingRequestSet())
      .external_wait();
  Processor::register_task_by_kind(p.kind(), false /*!global*/, WORK_TASK,
