...
TEST_F(HPPMemoryQueryTest, HasCapacityFilter)
{
  ...
  large_query.has_capacity(5ULL * 1024 * 1024 * 1024);
  EXPECT_EQ(large_query.count(), 1); // Only system memory (8GB) meets this

  // Query for memories with at least 2GB capacity
  HPP::Machine::MemoryQuery medium_query(machine);
  medium_query.has_capacity(2ULL * 1024 * 1024 * 1024);
  ...
  huge_query.has_capacity(10ULL * 1024 * 1024 * 1024);
  EXPECT_EQ(huge_query.count(), 0); // No memories meet this

  // Verify capacity constraints
  for(const auto &mem : large_query) {
    EXPECT_GE(mem.capacity(), 5ULL * 1024 * 1024 * 1024);
  }
  for(const auto &mem : medium_query) {
    EXPECT_GE(mem.capacity(), 2ULL * 1024 * 1024 * 1024);
  }
}
...
TEST_F(HPPMemoryQueryTest, EmptyResults)
{
  ...
  HPP::Machine::MemoryQuery query(machine);
  query.has_capacity(100ULL * 1024 * 1024 * 1024); // 100GB
...
TEST_F(HPPMemoryQueryTest, MultipleFilterChaining)
{
  ...
  chained_query.only_kind(HPP::Memory::Kind::SYSTEM_MEM)
      .local_address_space()
      .has_capacity(1ULL * 1024 * 1024 * 1024); // At least 1GB
...
TEST_F(HPPMemoryQueryTest, CapacityFilteringEdgeCases)
{
  ...
  exact_query.has_capacity(4ULL * 1024 * 1024 * 1024); // Exactly 4GB
  EXPECT_EQ(exact_query.count(), 2);                   // Both memories have >= 4GB

  // Test with capacity between the two memory sizes
  HPP::Machine::MemoryQuery between_query(machine);
  between_query.has_capacity(6ULL * 1024 * 1024 * 1024); // 6GB
  EXPECT_EQ(between_query.count(), 1); // Only system memory (8GB) meets this
}
...
TEST_F(HPPMemoryQueryTest, KindAndCapacityCombined)
{
  ...
  sysmem_large_query.only_kind(HPP::Memory::Kind::SYSTEM_MEM)
      .has_capacity(5ULL * 1024 * 1024 * 1024); // 5GB
  EXPECT_EQ(sysmem_large_query.count(), 1);     // System memory (8GB) meets this
  ...
  gpumem_small_query.only_kind(HPP::Memory::Kind::GPU_FB_MEM)
      .has_capacity(2ULL * 1024 * 1024 * 1024); // 2GB
  EXPECT_EQ(gpumem_small_query.count(), 1);     // GPU memory (4GB) meets this
  ...
  gpumem_too_large_query.only_kind(HPP::Memory::Kind::GPU_FB_MEM)
      .has_capacity(5ULL * 1024 * 1024 * 1024); // 5GB
  EXPECT_EQ(gpumem_too_large_query.count(), 0); // GPU memory (4GB) doesn't meet this
}
