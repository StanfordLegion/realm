...
// TEST_F(CInstCreateFromInstanceLayoutTest, CreateSuccess)
// {
...
//   realm_memory_t memory = ID::make_memory(0, 0).convert<Memory>();
...
//   space.num_dims =  1;
//   space.coord_type = REALM_COORD_TYPE_INT;
//   realm_field_layout_t field_layout[2];
//   field_layout[0].field_id = 0;
//   field_layout[0].size_in_bytes = sizeof(double);
//   field_layout[0].rel_offset = 0;
//   field_layout[1].field_id = 1;
//   field_layout[1].size_in_bytes = sizeof(int);
//   field_layout[1].rel_offset = sizeof(double) * 10;
//   instance_layout.field_layouts = field_layout;
//   instance_layout.num_fields = 2;
//   instance_layout.alignment_reqd = 32;
//   instance_layout.space = space;
//   int dim_order[1] = {0};
//   instance_layout.dim_order = dim_order;
...
//   realm_status_t status = realm_region_instance_create_from_instance_layout(runtime, &instance_layout, memory, nullptr, nullptr,
//                                                                             REALM_NO_EVENT, &inst, &event);

//   EXPECT_EQ(status, REALM_SUCCESS);
//   EXPECT_EQ(RegionInstance(inst).exists(), true);

//   // get the instance layout from the instance and check if it is the same as the input
//   RegionInstanceImpl *r_impl = runtime_impl->get_instance_impl(inst);
//   const InstanceLayout<1> *instance_layout_cxx = dynamic_cast<const InstanceLayout<1> *>(r_impl->metadata.layout);
//   ASSERT_TRUE(instance_layout_cxx != nullptr);
//   EXPECT_EQ(instance_layout_cxx->alignment_reqd, instance_layout.alignment_reqd);
//   Rect<1, int> bounds = instance_layout_cxx->space.bounds;
//   EXPECT_EQ(bounds.lo[0], lower_bound[0]);
//   EXPECT_EQ(bounds.hi[0], upper_bound[0]);
//   EXPECT_EQ(instance_layout_cxx->fields.size(), instance_layout.num_fields);
//   for(auto &field : instance_layout_cxx->fields) {
//     EXPECT_EQ(field.first, field_layout[field.first].field_id);
//     EXPECT_EQ(field.second.size_in_bytes, field_layout[field.first].size_in_bytes);
//   }
