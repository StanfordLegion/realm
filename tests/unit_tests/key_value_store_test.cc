...
static bool mock_cas(const void *key, size_t key_size, void *expected, size_t *expected_size,
...
protected:
  ...
  void InitVtable(bool with_put = true, bool with_get = true,
                  bool with_bar = false, bool with_cas = false)
  {
...
TEST_F(KeyValueStoreTestBase, CAS_CreateNew)
{
  ...
  bool result = runtime_impl->key_value_store_cas(key, strlen(key), &expected,
...
TEST_F(KeyValueStoreTestBase, Integration_ElasticWithCAS)
{
  ...
  bool result = runtime_impl->key_value_store_cas(counter_key, strlen(counter_key), &expected,
                                                   &expected_size, &desired, sizeof(desired));
  EXPECT_TRUE(result);
}
