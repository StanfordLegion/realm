...
protected:
  ...
  void setupBasicMachineModel()
  {
    ...
    setup.mem_infos = {
        {0, Memory::Kind::SYSTEM_MEM, 8ULL * 1024 * 1024 * 1024,
         0}, // 8GB system mem on node 0
        {1, Memory::Kind::GPU_FB_MEM, 4ULL * 1024 * 1024 * 1024,
         0} // 4GB GPU mem on node 0
    };
...
TEST_F(HPPMachineTest, GetLocalProcessorsByKind)
{
  ...
  std::set<HPP::Processor> gpu_processors;

  machine.get_local_processors_by_kind(cpu_processors, HPP::Processor::Kind::LOC_PROC);
  machine.get_local_processors_by_kind(gpu_processors, HPP::Processor::Kind::TOC_PROC);

  // Should have 2 CPUs and 1 GPU on local node
  ...
  for(const auto &proc : gpu_processors) {
...
TEST_F(HPPMachineTest, GetMemoriesByCapacity)
{
  ...
  // Get memories with at least 6GB capacity
  machine.get_memories_by_capacity(6ULL * 1024 * 1024 * 1024, large_memories);

  // Get memories with at least 2GB capacity
  machine.get_memories_by_capacity(2ULL * 1024 * 1024 * 1024, small_memories);

  // Should have 1 large memory (8GB system memory) and 2 small memories (all memories)
...
TEST_F(HPPMachineTest, MemoryQueryHasCapacity)
{
  ...
  // Query for memories with at least 6GB
  HPP::Machine::MemoryQuery large_query(machine);
  large_query.has_capacity(6ULL * 1024 * 1024 * 1024);
  EXPECT_EQ(large_query.count(), 1); // 1 memory with 8GB

  // Query for memories with at least 5GB
  HPP::Machine::MemoryQuery medium_query(machine);
  medium_query.has_capacity(5ULL * 1024 * 1024 * 1024);
  EXPECT_EQ(medium_query.count(), 1); // Still 1 memory (GPU has 4GB, so excluded)

  // Query for memories with at least 3GB
  HPP::Machine::MemoryQuery small_query(machine);
  small_query.has_capacity(3ULL * 1024 * 1024 * 1024);
...
TEST_F(HPPMachineTest, HasAffinityProcessorMemory)
{
  HPP::Machine machine = HPP::Machine::get_machine();
...
TEST_F(HPPMachineTest, HasAffinityMemoryMemory)
{
  HPP::Machine machine = HPP::Machine::get_machine();
...
TEST_F(HPPMachineTest, GetProcMemAffinity)
{
  HPP::Machine machine = HPP::Machine::get_machine();
...
TEST_F(HPPMachineTest, GetMemMemAffinity)
{
  HPP::Machine machine = HPP::Machine::get_machine();
...
TEST_F(HPPMachineTest, EmptyQueryResults)
{
  ...
  HPP::Machine::ProcessorQuery query(machine);
  query.only_kind(static_cast<HPP::Processor::Kind>(999)); // Invalid kind
