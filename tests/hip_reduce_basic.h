...
#ifndef HIP_REDUCE_BASIC_H
#define HIP_REDUCE_BASIC_H

#include <cstdint>

// Same-type integer add reduction: LHS == RHS == uint32_t.
// sizeof_lhs == sizeof_rhs satisfies the fast-path condition in progress_xd.
// Because only basic kernels are registered (advanced and transpose slots
// remain null), the has_fast_kernels guard causes dispatch to fall back to
// the basic stride loop instead of crashing.
...
public:
  ...
  // has_hip_reductions = false: realm.register_reduction<T>() must NOT
  // auto-register HIP kernels; we manually register only the basic 4.
