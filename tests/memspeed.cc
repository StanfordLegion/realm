...
void memspeed_cpu_task(const void *args, size_t arglen, const void *userdata,
                       size_t userlen, Processor p)
{
...
void memspeed_gpu_task(const void *args, size_t arglen, const void *userdata,
                       size_t userlen, Processor p)
{
  const SpeedTestArgs &cargs = *(const SpeedTestArgs *)args;
                    ...
                    Processor p)
{
  ...
  for(Machine::MemoryQuery::iterator it = Machine::MemoryQuery(machine).begin(); it;
    ...
    if(capacity < (TestConfig::buffer_size *
      ...
      log_app.info() << "skipping memory " << m << " (kind=" << m.kind()
  ...
  if(TestConfig::do_tasks) {
    for(std::vector<Memory>::const_iterator it = memories.begin(); it != memories.end();
      ...
      RegionInstance inst;
      RegionInstance::create_instance(inst, m, d, std::vector<size_t>(1, sizeof(void *)),
      ...
      for(Machine::ProcessorQuery::iterator it2 = pq.begin(); it2; ++it2) {
        ...
        if(supported_proc_kinds.count(p.kind()) == 0) {
          log_app.info() << "processor " << p << " is of unsupported kind " << p.kind()
                         << " - skipping";
          continue;
        }
  ...
  if(TestConfig::do_copies) {
    ...
    if(TestConfig::sparse_chunk > 0) {
      ...
      for(size_t ofs = 0; ofs <= (elements - TestConfig::sparse_chunk);
