...
void switch_task(const void *args, size_t arglen, const void *userdata, size_t userlen,
                 Processor p)
{
...
void sleep_task(const void *args, size_t arglen, const void *userdata, size_t userlen,
                Processor p)
{
...
void priority_child_task(const void *args, size_t arglen, const void *userdata,
...
void priority_master_task(const void *args, size_t arglen, const void *userdata,
                          size_t userlen, Processor p)
{
  int counter = 55;
  ...
  targs.barrier = barrier;
  {
    ...
    Event e = p.spawn(PRIORITY_CHILD_TASK, &targs, sizeof(targs), Event::NO_EVENT, 1);
...

  {
    ...
    Event e = p.spawn(PRIORITY_CHILD_TASK, &targs, sizeof(targs), Event::NO_EVENT, 0);
    finish_events.push_back(e);
  }

  {
    ...
    targs.new_priority = 1;
    Event e = p.spawn(PRIORITY_CHILD_TASK, &targs, sizeof(targs), Event::NO_EVENT, 2);
    finish_events.push_back(e);
  }
  ...
  std::set<Processor::Kind> seen;
  {
    ...
    for(std::set<Processor>::const_iterator it = all_processors.begin();
      ...
      if(num_iterations > 0) {
        ...
        printf("switch: proc " IDFMT
               " (kind=%d) finished: elapsed=%5.2fs time/switch=%6.0fns\n",
      ...
      if(sleep_useconds > 0) {
        ...
        double elapsed = t_end - t_start;
        printf("sleep: proc " IDFMT
               " (kind=%d) finished: elapsed=%5.2fs expected=%5.2fs\n",
