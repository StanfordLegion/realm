...
public:
  ...
  Event gather(IndexSpace<N, T> is, FieldID ptr_id, const SRC &src, FieldID src_id,
               FieldID dst_id, bool oor_possible, CustomSerdezID serdez_id, Event wait_on,
               Processor p);

  template <typename FT, typename DST>
  Event scatter(IndexSpace<N, T> is, FieldID ptr_id, DST &dst, FieldID src_id,
                FieldID dst_id, bool oor_possible, bool aliasing_possible,
                CustomSerdezID serdez_id, Event wait_on, Processor p) const;

  template <typename FT, typename SRC, typename DST>
  Event range_copy(IndexSpace<N, T> is, FieldID srcptr_id, const SRC &src, FieldID src_id,
                   bool src_oor_possible, bool src_aliasing_possible,
                   const DistributedData<N, T> &dstptr, FieldID dstptr_id, DST &dst,
                   FieldID dst_id, bool dst_oor_possible, bool dst_aliasing_possible,
  ...
  for(size_t i = 0; i < pieces.size(); i++) {
    ...
    pieces[i].proc = p;
    {
      Event e = RegionInstance::create_instance(
          pieces[i].inst, m, pieces[i].space, fields, 0 /* SOA */, ProfilingRequestSet());
      events.push_back(e);
    }
...
void indirect_prof_task(const void *args, size_t arglen, const void *userdata,
                        size_t userlen, Processor p)
{
  ProfilingResponse resp(args, arglen);
  assert(resp.user_data_size() == sizeof(IndirectCopyProfResult));
  const IndirectCopyProfResult *result =
      static_cast<const IndirectCopyProfResult *>(resp.user_data());
  ...
  if(resp.get_measurement(copy_info)) {
    ...
    log_app.print() << "copy type " << result->copy_type << ", src_insts ("
                    << PrettyVector<RegionInstance>(copy_info.inst_info[0].src_insts)
                    << ") size " << copy_info.inst_info[0].src_insts.size()
                    << ", dst_insts ("
                    << PrettyVector<RegionInstance>(copy_info.inst_info[0].dst_insts)
                    << ") size " << copy_info.inst_info[0].dst_insts.size()
                    << ", src_fid " << copy_info.inst_info[0].src_fields[0]
                    << ", dst_fid " << copy_info.inst_info[0].dst_fields[0]
                    << ", src_indirect_inst "
                    << copy_info.inst_info[0].src_indirection_inst << " fid "
                    << copy_info.inst_info[0].src_indirection_field
                    << ", dst_indirect_inst "
                    << copy_info.inst_info[0].dst_indirection_inst << " fid "
                                    ...
                                    CustomSerdezID serdez_id, Event wait_on, Processor p)
{
  ...
  const std::map<Point<N, T>, Maybe<Point<SRC::_N, typename SRC::_T>>> &ptrref =
      get_ref_data<Point<SRC::_N, typename SRC::_T>>(ptr_id);
...
Event DistributedData<N, T>::range_copy(IndexSpace<N, T> is, FieldID srcptr_id,
                                        ...
                                        bool src_oor_possible, bool src_aliasing_possible,
                                        const DistributedData<N, T> &dstptr,
                                        FieldID dstptr_id, DST &dst, FieldID dst_id,
                                        ...
                                        Event wait_on, Processor p) const
{
  ...
  for(typename std::vector<Piece>::const_iterator it = pieces.begin(); it != pieces.end();
    ...
    for(typename std::vector<Piece>::const_iterator it2 = dstptr.pieces.begin();
      ...
      for(typename std::vector<typename SRC::Piece>::const_iterator it3 =
              src.pieces.begin();
          it3 != src.pieces.end(); ++it3) {
        src_ind.spaces.push_back(it3->space);
        src_ind.insts.push_back(it3->inst);
      }

      for(typename std::vector<typename DST::Piece>::const_iterator it3 =
              dst.pieces.begin();
          it3 != dst.pieces.end(); ++it3) {
...
bool range_copy_test(const std::vector<Memory> &mems, T size1, T2 size2, T3 size3,
                     int pieces1, int pieces2, int pieces3, Processor p)
{
  Rect<N, T> r1;
  Rect<N2, T2> r2;
  Rect<N3, T3> r3;
  ...
  region1
      .template fill<Rect<N2, T2>>(
          is1, FID_RANGE1,
          [=](Point<N, T> p) -> Rect<N2, T2> { return Rect<N2, T2>::make_empty(); },
          Event::NO_EVENT)
      .wait();
  // region1.template fill<Rect<N3,T3> >(is1, FID_RANGE2, [=](Point<N,T> p) -> Rect<N3,T3>
  // { return Rect<N3,T3>::make_empty(); }, Event::NO_EVENT).wait();
  region1
      .template fill<Rect<N3, T3>>(
          is1, FID_RANGE2, ConstantFiller<Rect<N3, T3>>(Rect<N3, T3>::make_empty()),
          Event::NO_EVENT)
      .wait();
  region1
      .template fill<Rect<N2, T2>>(Rect<N, T>(Point<N, T>(1), Point<N, T>(1)), FID_RANGE1,
                                   ConstantFiller<Rect<N2, T2>>(Rect<N2, T2>(
                                       Point<N2, T2>(1, 0), Point<N2, T2>(1, 1))),
                                   Event::NO_EVENT)
      .wait();
  region1
      .template fill<Rect<N2, T2>>(Rect<N, T>(Point<N, T>(3), Point<N, T>(3)), FID_RANGE1,
                                   ConstantFiller<Rect<N2, T2>>(Rect<N2, T2>(
                                       Point<N2, T2>(1, 2), Point<N2, T2>(1, 3))),
                                   Event::NO_EVENT)
      .wait();
  region1
      .template fill<Rect<N3, T3>>(Rect<N, T>(Point<N, T>(1), Point<N, T>(1)), FID_RANGE2,
                                   ConstantFiller<Rect<N3, T3>>(Rect<N3, T3>(
                                       Point<N3, T3>(1, 1, 1), Point<N3, T3>(1, 2, 1))),
                                   Event::NO_EVENT)
      .wait();
  region1
      .template fill<Rect<N3, T3>>(Rect<N, T>(Point<N, T>(3), Point<N, T>(3)), FID_RANGE2,
                                   ConstantFiller<Rect<N3, T3>>(Rect<N3, T3>(
                                       Point<N3, T3>(2, 1, 1), Point<N3, T3>(2, 2, 1))),
                                   Event::NO_EVENT)
      .wait();

  region2.template fill<DT>(is2, FID_DATA1, RegularFiller<DT>(1000), Event::NO_EVENT)
      .wait();

  region3.template fill<DT>(is3, FID_DATA1, RegularFiller<DT>(2000), Event::NO_EVENT)
      .wait();
#if 0
  ...
  region1
      .template range_copy<DT>(
          is1, FID_RANGE1, region2, FID_DATA1, false /*!src_oor_possible*/,
          true /*src_aliasing_possible*/, region1, FID_RANGE2, region3, FID_DATA1,
          false /*!dst_oor_possible*/, true /*dst_aliasing_possible*/, Event::NO_EVENT, p)
      .wait();
                    ...
                    Processor p)
{
  ...
  if(!scatter_gather_test<1, int, 2, int, float>(mems, TestConfig::size1,
                                                 ...
                                                 TestConfig::pieces2, p))
    ok = false;

  // really big (non-power-of-2) fields
  typedef Pad<float, 2000> BigFloat;
  if(!scatter_gather_test<1, int, 2, int, BigFloat>(
         mems, TestConfig::size1, TestConfig::size2, TestConfig::pieces1,
         TestConfig::pieces2, p))
    ok = false;

  // serdez
  if(do_serdez && !scatter_gather_test<1, int, 2, int, float>(
                      mems, TestConfig::size1, TestConfig::size2, TestConfig::pieces1,
                      TestConfig::pieces2, p, SERDEZ_WRAP_FLOAT))
    ok = false;

  if(!range_copy_test<1, int, 2, int, 3, int, float>(
         mems, 4, 4, 4, 1, 1 /*TestConfig::pieces1*/, 1 /*TestConfig::pieces2*/, p))
    ok = false;
...
int main(int argc, char **argv)
{
  ...
  Processor::register_task_by_kind(Processor::LOC_PROC, false /*!global*/,
                                   INDIRECT_PROF_TASK, CodeDescriptor(indirect_prof_task),
