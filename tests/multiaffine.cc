...
void ptr_write_task_cpu(const void *args, size_t arglen, const void *userdata,
                        size_t userlen, Processor p)
{
  const PtrWriteTaskArgs<N, T> &targs =
      *static_cast<const PtrWriteTaskArgs<N, T> *>(args);
  ...
  for(IndexSpaceIterator<N, T> it(targs.space); it.valid; it.step()) {
    for(PointInRectIterator<N, T> pit(it.rect); pit.valid; pit.step()) {
      ...
      log_app.debug() << "write " << pit.p << ": " << std::hex
...
template <int N, typename T>
bool test_case(Processor curr_proc, Processor write_proc, IndexSpace<N, T> space,
               int num_pieces, int seed, int test_id)
{
  // choose memories for instances
  Memory m_check =
      Machine::MemoryQuery(Machine::get_machine()).best_affinity_to(curr_proc).first();
  Memory m_write =
      Machine::MemoryQuery(Machine::get_machine()).best_affinity_to(write_proc).first();
  ...
  RegionInstance inst_check;
  {
    ...
    InstanceLayoutGeneric *ilg = InstanceLayoutGeneric::choose_instance_layout<N, T>(
        space, InstanceLayoutConstraints(field_sizes, 0 /*SOA*/), dim_order);
  ...
  for(int i = 1; i < num_pieces; i++) {
    ...
    split_idx = (pieces[to_split].lo[split_dim] +
                 PRNG::rand_int(
                     ...
                     (pieces[to_split].hi[split_dim] - pieces[to_split].lo[split_dim])));
  ...
  RegionInstance inst_write;
  {
    ...
    for(int i = 0; i < N; i++)
      dim_order[i] = i;
    InstanceLayoutGeneric *ilg = InstanceLayoutGeneric::choose_instance_layout<N, T>(
        space, pieces, InstanceLayoutConstraints(field_sizes, 0 /*SOA*/), dim_order);
  ...
  size_t errors = 0;
  {
    ...
    for(int i = 0; i < num_pieces; i++) {
      ...
      for(PointInRectIterator<N, T> pit(pieces[i]); pit.valid; pit.step()) {
        ...
        if(val != exp) {
          log_app.error() << "at " << pit.p << ": expected " << std::hex << exp
                          << ", actual " << val << std::dec;
          errors++;
        }
      }
                    ...
                    Processor p)
{
  ...
  if((TestConfig::dim_mask & 1) != 0) {
    ...
    if(!test_case(p, proc_write, IndexSpace<1>(bounds), 8, TestConfig::random_seed,
                  test_id))
      errors++;
  }
  ...
  if((TestConfig::dim_mask & 2) != 0) {
    ...
    bounds.lo[1] = 0;
    bounds.hi[1] = (1 << ly2) - 1;
    if(!test_case(p, proc_write, IndexSpace<2>(bounds), 8, TestConfig::random_seed,
                  test_id))
      errors++;
  }
...
int main(int argc, char **argv)
{
  ...
  Processor::register_task_by_kind(
      Processor::LOC_PROC, false /*!global*/, PTR_WRITE_TASK_BASE + 1,
      CodeDescriptor(ptr_write_task_cpu<1, int>), ProfilingRequestSet(), 0, 0)
      .wait();
  Processor::register_task_by_kind(
      Processor::LOC_PROC, false /*!global*/, PTR_WRITE_TASK_BASE + 2,
      CodeDescriptor(ptr_write_task_cpu<2, int>), ProfilingRequestSet(), 0, 0)
      .wait();
