                      ...
                      Memory mem = Memory::NO_MEMORY)
{
  ...
  if(src.address_space() != Processor::get_executing_processor().address_space()) {
    ...
    e = Event::merge_events(e,
                            RegionInstance::create_instance(dst, mem, range, field_sizes,
...
static Event put_data(RegionInstance dst, Rect<1> range, size_t field_id,
                      size_t field_size, RegionInstance src, Event e = Event::NO_EVENT)
{
  if(dst != src) {
    std::vector<CopySrcDstField> src_desc(1), dst_desc(1);
    e = Event::merge_events(e, dst.fetch_metadata(Processor::get_executing_processor()));
    src_desc[0].set_field(src, field_id, field_size);
    dst_desc[0].set_field(dst, field_id, field_size);
...
static void top_level_task(const void *args, size_t arglen, const void *userdata,
                           size_t userlen, Processor p)
{
  ...
  Memory memory =
      Machine::MemoryQuery(Machine::get_machine())
          .local_address_space()
          .has_affinity_to(p)
          .has_capacity((sizeof(Reservation) + sizeof(size_t)) * domain.volume())
          .first();

  std::vector<size_t> reservation_field_sizes(1, sizeof(Reservation));
  std::vector<size_t> value_field_sizes(1, sizeof(size_t));

  Event rsrv_inst_event =
      RegionInstance::create_instance(reservation_instance, memory, idx_space,
                                      reservation_field_sizes, 0, ProfilingRequestSet());

  Event value_inst_event =
      RegionInstance::create_instance(value_instance, memory, idx_space,
                                      reservation_field_sizes, 0, ProfilingRequestSet());

  {
    ...
    for(size_t i = 0; i < procs.count(); ++target_proc, ++i) {
      res_params.idx = i;
      events[res_params.idx] = target_proc->spawn(MAKE_RESERVATIONS_TASK, &res_params,
    ...
    events.back() = idx_space.fill(fill_desc, ProfilingRequestSet(), &fill_value,
...

  {
    ...
    for(size_t i = 0; i < chain_params.num_reservations; i++) {
      ...
      if(value != params.tasks_per_processor * procs.count()) {
        log_app.error("%s(%d): Value[%llu]=%llu", __FILE__, __LINE__,
...
static void make_reservations_task(const void *args, size_t arglen, const void *userdata,
                                   size_t userlen, Processor p)
{
  ...
  Rect<1> update_rect =
      Rect<1>(params.idx * params.stride, ((params.idx + 1) * params.stride) - 1);

  {
    ...
    RegionInstance::create_instance(tmp_instance, local_memory, update_rect,
                                    tmp_inst_field_sizes, 0, ProfilingRequestSet())
        .wait();
                                    ...
                                    size_t userlen, Processor p)
{
  ...
  Event e = get_data(params.reservation_instance, Rect<1>(0, params.num_reservations - 1),
                                     ...
                                     const void *userdata, size_t userlen, Processor p)
{
  ...
  RegionInstance local_rsrv_instance;
  get_data(params.reservation_instance, Rect<1>(0, params.num_reservations - 1), 0,
  ...
  for(size_t i = 0; i < params.num_reservations; i++) {
    ...
    // instance.  No other processor may update this at the same time
    Event e = res.acquire();
    events[i] = p.spawn(UPDATE_VALUE_TASK, &update_params, sizeof(update_params), e);
...
static void update_value_task(const void *args, size_t arglen, const void *userdata,
                              size_t userlen, Processor p)
{
  ...
  get_data(params.value_instance, region_rect, 0, sizeof(size_t), value_instance).wait();

  AffineAccessor<size_t, 1> values = AffineAccessor<size_t, 1>(value_instance, 0, 0);
...
int main(int argc, char **argv)
{
  ...
  events.push_back(Processor::register_task_by_kind(
      Processor::LOC_PROC, false, TOP_LEVEL_TASK, CodeDescriptor(top_level_task),
  ...
  events.push_back(Processor::register_task_by_kind(
      Processor::LOC_PROC, false, UPDATE_VALUE_TASK, CodeDescriptor(update_value_task),
      ProfilingRequestSet(), 0, 0));
