                     ...
                     Rect2 bounds)
{
  ...
  copy2D(src_base, dst_base, src_stride / sizeof(DTYPE), src_offset[0],
         src_offset[0] + size[0], src_offset[1], src_offset[1] + size[1],
                  ...
                  Processor p)
{
  ...
  get_base_and_stride(a.private_inst, FID_INPUT, private_base_input,
                      private_stride_input);
  get_base_and_stride(a.private_inst, FID_OUTPUT, private_base_output,
                      private_stride_output);
  ...
  stencil(private_base_input, private_base_output, weights,
          private_stride_input / sizeof(DTYPE), interior_offset[0],
          interior_offset[0] + interior_size[0], interior_offset[1],
...
#undef WEIGHT
...

  {
    ...
    Event check_done =
        p.spawn(CHECK_TASK, &check_args, sizeof(check_args), increment_done);
    check_done.wait();
  }
                    ...
                    Processor p)
{
  ...
  Barrier first_start_bar = Barrier::create_barrier(
      nshards, REDOP_MIN, &RedopMin::identity, sizeof(RedopMin::identity));
  Barrier first_stop_bar = Barrier::create_barrier(
      nshards, REDOP_MIN, &RedopMin::identity, sizeof(RedopMin::identity));
