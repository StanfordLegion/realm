...
static void output_configuration(size_t total_size)
{
  printf("BENCHMARK_CONFIGURATION {enable_remote_copy:%d, graph_type:%d, "
         "num_iterations:%zu, num_samples:%zu, size_per_copy:%zu, total_copy_size:%zu}\n",
         TestConfig::enable_remote_copy, TestConfig::graph_type, TestConfig::num_iterations,
         TestConfig::num_samples, TestConfig::size*sizeof(ElementType), total_size);
}
                              ...
                              Processor p) 
{
  ...
  if(memories.size() == 0) {
    log_app.error("No memory has been tested");
    abort();
  }
  ...
  output_configuration(total_size_bytes);
#ifdef BENCHMARK_USE_JSON_FORMAT
  std::cout << "RESULT {name:graph_bandwidth, value:" << total_size_bytes / (1000.0 * graph_time.get_average()) << ", unit:+GB/s}" << std::endl;
#else
  std::cout << "RESULT graph_bandwidth=/" << total_size_bytes / (1000.0 * graph_time.get_average()) << " +GB/s" << std::endl;
#endif
