...
#undef DEFINE_FNPTR
    ...
    // Header used for AM_ID_REPLY messages.
    // Carries the sender node id and the pointer to the sender's RemoteComp.
    struct ReplyHdr {
    ...
    static uint32_t compute_packet_crc(const void *header_base, size_t header_size,
                                       size_t payload_size)
    {
    ...
    static void insert_packet_crc(UCPMsgHdr *ucp_msg_hdr, size_t realm_hdr_size,
                                  size_t payload_size)
    {
      // the crc field itself should not be included in CRC checksum
      ucp_msg_hdr->crc = compute_packet_crc(
          &ucp_msg_hdr->src,
          sizeof(*ucp_msg_hdr) - sizeof(ucp_msg_hdr->crc) + realm_hdr_size, payload_size);
    }

    static void verify_packet_crc(const UCPMsgHdr *ucp_msg_hdr, size_t realm_hdr_size,
                                  size_t payload_size)
    {
      // the crc field itself should not be included in CRC checksum
    ...
    static void
    read_env_var_update_map(const std::string &ev, const std::string &default_val,
                            std::string *var,
    ...
    void UCPPoller::begin_polling() { make_active(); }
    ...
    void UCPPoller::add_worker(UCPWorker *worker) { workers.push_back(worker); }
    ...
    bool UCPInternal::create_eps(uint8_t priority)
    {
      ...
      rc = ucc_comm->UCC_Allgather(&self_max_addrlen, sizeof(self_max_addrlen),
                                   UCC_DT_UINT8, all_max_addrlen.data(),
                                   sizeof(self_max_addrlen) * ucc_comm->get_world_size(),
                                   UCC_DT_UINT8);

      CHKERR_JUMP(rc != UCC_OK, "failed to allgather max worker address length", log_ucp,
                  err);

      max_addrlen = *std::max_element(all_max_addrlen.begin(), all_max_addrlen.end());

      // get the number of rx workers created by each rank
      rc = ucc_comm->UCC_Allgather(
          &self_num_rx_workers, sizeof(self_num_rx_workers), UCC_DT_UINT8,
          all_num_rx_workers.data(),
          sizeof(self_num_rx_workers) * ucc_comm->get_world_size(), UCC_DT_UINT8);
      CHKERR_JUMP(rc != UCC_OK, "failed to allgather the number of workers", log_ucp,
                  err);

      max_num_rx_workers =
          *std::max_element(all_num_rx_workers.begin(), all_num_rx_workers.end());
      ...
      rc = ucc_comm->UCC_Allgather(
          ...
          UCC_DT_UINT8);
      CHKERR_JUMP(rc != UCC_OK, "failed to allgather worker device indices", log_ucp,
                  err);
        ...
        for(size_t i = 0; i < all_num_rx_workers[peer]; i++) {
          ...
          for(const auto &context : ucp_contexts) {
            ...
            CHKERR_JUMP(!worker->ep_add(peer, peer_worker_addr, peer_dev_index),
                        "failed to add ep. peer " << peer << " peer_dev_index "
                                                  << peer_dev_index,
                        log_ucp, err);

            log_ucp_ep.debug() << "added ep target " << peer << " context " << &context
                               << " worker " << worker << " remote dev_index "
                               << peer_dev_index;
          }
        }
      }
    ...
    out:
      for(auto &wi : self_rx_workers_info) {
        UCP_FNPTR(ucp_worker_release_address)(wi.worker->get_ucp_worker(), wi.addr);
      }
      return ret;
    ...
    bool UCPInternal::create_workers()
    {
      ...
      std::vector<UCPWorker::Type> wts = {UCPWorker::WORKER_TX, UCPWorker::WORKER_RX};

      // create num_priorities workers per worker type (tx, rx) per context
      context = get_context_host();
      for(const auto &wt : wts) {
        for(uint8_t priority = 0; priority < config.num_priorities; priority++) {
          worker = new UCPWorker(
              context, wt, AM_ALIGNMENT, config.use_wakeup, config.prog_boff_max,
        ...
        for(const auto &wt : wts) {
          for(uint8_t priority = 0; priority < config.num_priorities; priority++) {
            worker = new UCPWorker(
                ...
                (wt == UCPWorker::WORKER_TX) ? UCS_THREAD_MODE_MULTI
                                             : UCS_THREAD_MODE_SERIALIZED,
                sizeof(Request), alignof(Request), 1, 1, 0,
                0, /* we never get payload buffer from GPU contexts */
                config.mmp_max_obj_size, config.mpool_leakcheck);

            CHKERR_JUMP(!worker->init(),
                        "failed to init worker for device context " << context, log_ucp,
                        err);
        ...
        for(UCPWorker *worker : get_rx_workers(&context)) {
          ...
          for(const auto &ah : am_handlers) {
            if(!worker->set_am_handler(ah.id, ah.cb, &am_handlers_args.back())) {
              log_ucp.error() << "failed to set ucp " << ah.name << " handler for worker "
                              << worker;
              return false;
            }
          }
...
#ifdef REALM_USE_CUDA
    bool UCPInternal::init_ucp_contexts(const std::unordered_set<Cuda::GPU *> &gpus)
    {
    ...
    bool UCPInternal::init_ucp_contexts()
    {
      ...
      if(config.mmp_max_obj_size < config.pbuf_mp_thresh) {
        log_ucp.warning() << "WARNING: mmp_max_obj_size " << config.mmp_max_obj_size
                          << " lt pbuf_mp_thresh " << config.pbuf_mp_thresh;
      }

      read_env_var_update_map("UCX_NET_DEVICES", "", &config.host_nics, &ev_map);
      read_env_var_update_map("UCX_TLS", "", &config.tls_host, &ev_map);
      read_env_var_update_map("UCX_IB_SEG_SIZE", std::to_string(IB_SEG_SIZE),
                              &config.ib_seg_size, &ev_map);
      read_env_var_update_map("UCX_ZCOPY_THRESH", std::to_string(ZCOPY_THRESH_HOST),
                              &config.zcopy_thresh_host, &ev_map);
      ...
      if(!config.gpu_nics.empty()) {
        ...
        while(std::getline(stream, token, ';')) {
          ...
          if((c != std::string::npos) && (c > 0)) {
            ...
            log_ucp.info() << "NICs for CUDA device " << cuda_device_index << ": "
                           << nics;
          }
        }
      }
      ...
      read_env_var_update_map("UCX_ZCOPY_THRESH", std::to_string(ZCOPY_THRESH_DEV),
                              &config.zcopy_thresh_dev, &ev_map);
      ...
      CHKERR_JUMP(!create_workers(), "failed to create workers", log_ucp,
                  err_fin_contexts);

      CHKERR_JUMP(!set_am_handlers(), "failed to set ucp am handlers", log_ucp,
                  err_destroy_workers);

      CHKERR_JUMP(!create_eps(), "failed to create ucp end points", log_ucp,
                  err_destroy_workers);

      CHKERR_JUMP(!create_pollers(), "failed to create ucp pollers", log_ucp,
                  err_destroy_workers);
      ...
      for(UCPContext &context : ucp_contexts) {
        ...
        log_ucp_ep.info() << "context " << &context << " num_eps " << num_eps(context);
      }
...
#undef STRINGIFY
    ...
    bool UCPInternal::init(const Config &_config)
    {
      ...
      if(UCP_VERSION(major, minor) < REALM_UCP_API_VERSION_MIN) {
        log_ucp.warning()
            << "The UCX network module requires UCX " REALM_UCP_API_VERSION_MIN_STR
               " or above";
        goto err_version;
      }

      // RealmCallbackArgs mpool
      rcba_mp = new MPool("rcba_mp", config.mpool_leakcheck, sizeof(RealmCallbackArgs),
                          alignof(RealmCallbackArgs), 0);
    ...
    ucs_status_t UCPInternal::am_remote_comp_handler(void *arg, const void *header,
                                                     ...
                                                     const ucp_am_recv_param_t *param)
    {
      ...
      // The header contains the sender's node id and the pointer to the corresponding
      // remote_comp
      assert(header_size == sizeof(ReplyHdr));
      ReplyHdr rh;
      memcpy(&rh, header, sizeof(rh));
      remote_comp = rh.remote_comp;
    ...
    void UCPInternal::am_send_reply_comp_handler(void *request, ucs_status_t status,
                                                 void *user_data)
    {
                                            ...
                                            IncomingMessageManager::CallbackData cb_data2)
    {
      ...
      if(cb_args->remote_comp != nullptr) {
        ...
        ReplyHdr rh;
        rh.remote_comp = cb_args->remote_comp;
        rh.src = Network::my_node_id;
        req->ucp.am.header = &rh;
        req->ucp.am.header_size = sizeof(rh);

        // Attribute this reply to the correct peer for per-peer counters
        req->target_peer = sender;
        if((req->target_peer >= 0) && (req->target_peer != Network::my_node_id)) {
          req->internal->bump_peer(req->target_peer, [](PeerCounters &pc) {
            pc.outstanding.fetch_add(1, std::memory_order_relaxed);
          });
        }

        CHKERR_JUMP(!tx_worker->submit_req(&req->ucp), "failed to send am reply", log_ucp,
                    err_rel_req);
    ...
    bool UCPInternal::am_msg_recv_data_ready(UCPInternal *internal, UCPWorker *worker,
                                             const UCPMsgHdr *ucp_msg_hdr,
                                             size_t header_size, void *payload,
                                             size_t payload_size, int payload_mode)
    {
      ...
      bool completed = runtime->message_manager->add_incoming_message(
          ucp_msg_hdr->src, ucp_msg_hdr->msgid, realm_hdr, realm_hdr_size,
          PAYLOAD_COPY, // header is always copied
    ...
    void UCPInternal::am_rndv_recv_data_handler(void *request, ucs_status_t status,
                                                size_t length, void *user_data)
    {
      ...
      if(status == UCS_OK) {
        (void)internal->am_msg_recv_data_ready(
            internal, worker, ucp_msg_hdr, req->am_rndv_recv.header_size,
    ...
    ucs_status_t UCPInternal::am_msg_recv_handler(void *arg, const void *header,
                                                  size_t header_size, void *payload,
                                                  size_t payload_size,
                                                  const ucp_am_recv_param_t *param)
    {
      ...
      if(param->recv_attr & UCP_AM_RECV_ATTR_FLAG_RNDV) {
        ...
        param.op_attr_mask = UCP_OP_ATTR_FIELD_CALLBACK | UCP_OP_ATTR_FIELD_REQUEST |
                             UCP_OP_ATTR_FIELD_RECV_INFO | UCP_OP_ATTR_FIELD_MEMORY_TYPE;
        param.cb.recv_am = &am_rndv_recv_data_handler;
        param.request = req;
        param.recv_info.length = &recv_info_length;
#ifdef REALM_USE_CUDA
        param.memory_type =
            worker->get_context()->gpu ? UCS_MEMORY_TYPE_CUDA : UCS_MEMORY_TYPE_HOST;
#else
        ...
        status_ptr = UCP_FNPTR(ucp_am_recv_data_nbx)(
            worker->get_ucp_worker(), data_desc, req->am_rndv_recv.payload,
      ...
      (void)internal->am_msg_recv_data_ready(internal, worker, ucp_msg_hdr, header_size,
    ...
    void UCPInternal::broadcast(NodeID root, const void *val_in, void *val_out,
                                size_t bytes)
    {
      ucc_status_t rc;
    ...
    void UCPInternal::gather(NodeID root, const void *val_in, void *vals_out,
                             size_t bytes)
    {
      ucc_status_t rc;
                                 ...
                                 std::vector<size_t> &lengths)
    {
      ...
      rc = ucc_comm->UCC_Allgather(&bytes, sizeof(bytes), UCC_DT_UINT8, lengths.data(),
                                   sizeof(bytes) * ucc_comm->get_world_size(),
                                   UCC_DT_UINT8);
      if(rc != UCC_OK) {
      ...
      rc = ucc_comm->UCC_Allgatherv(const_cast<char *>(val_in),
                                    sizes[ucc_comm->get_rank()], UCC_DT_UINT8,
                                    vals_out.data(), sizes, offsets, UCC_DT_UINT8);
    ...
    UCPWorker *UCPInternal::get_tx_worker(const UCPContext *context,
    ...
    UCPWorker *UCPInternal::get_rx_worker(const UCPContext *context,
                                          uint8_t priority) const
    {
    ...
    void UCPMessageImpl::am_local_failure_handler(Request *req, UCPInternal *internal)
    {
      ...
      log_ucp_am.debug() << "am_local_failure_handler invoked for request " << req;
    ...
    void UCPMessageImpl::am_local_comp_handler(void *request, ucs_status_t status,
                                               void *user_data)
    {
      Request *req = reinterpret_cast<Request *>(user_data);
      UCPInternal *internal = req->internal;
    ...
    bool UCPMessageImpl::send_slow_path(ucp_ep_h ep, size_t act_payload_size,
                                        uint32_t flags)
    {
