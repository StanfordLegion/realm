...
namespace Realm {
  ...
  namespace UCP {
    ...
    bool UCPWorker::progress()
    {
      ...
      return use_wakeup ? progress_with_wakeup() : progress_without_wakeup();
    }

    bool UCPWorker::set_am_handler(unsigned am_id, ucp_am_recv_callback_t cb, void *args)
    {
    ...
    bool UCPWorker::am_send_fast_path(ucp_ep_h ep, unsigned am_id, const void *header,
                                      size_t header_size, const void *payload,
                                      size_t payload_size, ucs_memory_type_t memtype)
    {
      ...
      param.op_attr_mask = UCP_OP_ATTR_FLAG_FORCE_IMM_CMPL |
                           UCP_OP_ATTR_FIELD_MEMORY_TYPE | UCP_OP_ATTR_FIELD_FLAGS;

      param.memory_type = memtype;
      param.flags = UCP_AM_SEND_FLAG_COPY_HEADER;

      scount.fetch_add_acqrel(1);
      status_ptr = UCP_FNPTR(ucp_am_send_nbx)(ep, am_id, header, header_size, payload,
                                              payload_size, &param);
    ...
    bool UCPWorker::submit_req(Request *req)
    {
      ...
      param.op_attr_mask = UCP_OP_ATTR_FIELD_REQUEST | UCP_OP_ATTR_FIELD_CALLBACK |
                           UCP_OP_ATTR_FIELD_USER_DATA | UCP_OP_ATTR_FIELD_MEMORY_TYPE |
                           UCP_OP_ATTR_FIELD_FLAGS;
      param.request = req;
      param.cb.send = req->cb, param.user_data = req->args,
      param.memory_type = req->memtype, param.flags = req->flags;
      ...
      case AM_SEND:
        param.flags |= UCP_AM_SEND_FLAG_COPY_HEADER;
        status_ptr = UCP_FNPTR(ucp_am_send_nbx)(req->ep, req->am.id, req->am.header,
                                                req->am.header_size, req->payload,
                                                req->payload_size, &param);
        CHKERR_JUMP(UCS_PTR_IS_ERR(status_ptr), "ucp_am_send_nbx failed", log_ucp, err);
        break;
      case PUT:
        status_ptr = UCP_FNPTR(ucp_put_nbx)(req->ep, req->payload, req->payload_size,
                                            req->rma.remote_addr, req->rma.rkey, &param);
        CHKERR_JUMP(UCS_PTR_IS_ERR(status_ptr), "ucp_put_nbx failed", log_ucp, err);
        break;
      case EP_FLUSH:
        status_ptr = UCP_FNPTR(ucp_ep_flush_nbx)(req->ep, &param);
        CHKERR_JUMP(UCS_PTR_IS_ERR(status_ptr), "ucp_ep_flush_nbx failed", log_ucp, err);
        break;
      default:
    ...
    UCPWorker::UCPWorker(const UCPContext *context_, Type type_, size_t am_alignment_,
      ...
      , pbuf_max_size(pbuf_max_size_)
    {
      ...
      request_mp = new MPool("request_mp", leak_check_, user_req_size_ + ucp_req_size,
                             user_req_alignment_, ucp_req_size);
      ...
      // simple malloc mpool
      mmp =
          new VMPool("internal_malloc_mp", leak_check_, mmp_max_obj_size_, am_alignment_);
    }
    ...
    bool UCPWorker::init()
    {
      ...
      worker_params.field_mask =
          UCP_WORKER_PARAM_FIELD_THREAD_MODE | UCP_WORKER_PARAM_FIELD_AM_ALIGNMENT;
      worker_params.thread_mode = thread_mode;
      worker_params.am_alignment = am_alignment;

      status = UCP_FNPTR(ucp_worker_create)(context->get_ucp_context(), &worker_params,
                                            &worker);
      CHKERR_JUMP(status != UCS_OK, "ucp_worker_create failed", log_ucp, err);

      worker_attr.field_mask =
          UCP_WORKER_ATTR_FIELD_THREAD_MODE | UCP_WORKER_ATTR_FIELD_MAX_AM_HEADER;

      status = UCP_FNPTR(ucp_worker_query)(worker, &worker_attr);
      CHKERR_JUMP(status != UCS_OK, "ucp_worker_query failed", log_ucp,
                  err_destroy_worker);
      ...
      initialized = true;
      log_ucp.info() << "initialized ucp worker " << this << " context " << context
    ...
    bool UCPContext::init(const std::unordered_map<std::string, std::string> &ev_map)
    {
      ...
      ucp_params.features = UCP_FEATURE_AM | UCP_FEATURE_RMA | UCP_FEATURE_WAKEUP;
      ...
      status = UCP_FNPTR(ucp_init_version)(UCP_API_MAJOR, UCP_API_MINOR, &ucp_params,
                                           ucp_config, &context);
      ...
      CHKERR_JUMP(status != UCS_OK, "ucp_context_query failed", log_ucp,
                  err_cleanup_context);
