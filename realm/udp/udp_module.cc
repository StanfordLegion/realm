...
namespace Realm {
                                     ...
                                     std::vector<std::string> &cmdline)
  {
    ...
    cp.add_option_int("-udp:port", config_udp_port);
    cp.add_option_string("-udp:address", config_udp_address);
    ...
    init(static_cast<uint16_t>(config_udp_port), config_udp_address);
  }
  ...
  void UDPModule::init(uint16_t base_port, const std::string &address)
  {
    ...
    sin.sin_port = htons(base_port + Network::my_node_id);
    ...
    for(NodeID nid = 0; nid <= Network::max_node_id; nid++) {
      if(nid != Network::my_node_id) {
        register_peer(nid, address, base_port + nid);
      }
    }
