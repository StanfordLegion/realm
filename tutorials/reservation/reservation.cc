                       ...
                       size_t userlen, Processor p)
{
  ...
  if(task_args.inst.address_space() != p.address_space()) {
    ...
    Event e2 =
        RegionInstance::create_instance(inst, cpu_mem, task_args.is, field_sizes,
...
void writer_task1(const void *args, size_t arglen, const void *userdata, size_t userlen,
                  Processor p)
{
  const WriterTaskArgs &task_args = *reinterpret_cast<const WriterTaskArgs *>(args);
...
void writer_task2(const void *args, size_t arglen, const void *userdata, size_t userlen,
                  Processor p)
{
  const WriterTaskArgs &task_args = *reinterpret_cast<const WriterTaskArgs *>(args);
               ...
               Processor p)
{
  ...
  Memory cpu_mem = Machine::MemoryQuery(Machine::get_machine())
                       ...
                       .first();
  assert(cpu_mem.exists());
  RegionInstance::create_instance(inst, cpu_mem, is, field_sizes, 0 /*block_size=SOA*/,
                                  ProfilingRequestSet())
      .wait();
  ...
  for(int i = 0; i < TestConfig::num_writers; i++) {
    ...
    if(TestConfig::distributed_reservation) {
      ...
      e = cpus[i % cpus.size()].spawn(WRITER_TASK2, &task_args, sizeof(WriterTaskArgs),
                                      start_event);
    } else {
      Event e1 = reservation.acquire(0, true, start_event);
      e = cpus[i % cpus.size()].spawn(WRITER_TASK1, &task_args, sizeof(WriterTaskArgs),
                                      e1);
  ...
  for(int i = 0; i < TestConfig::num_readers; i++) {
    ...
    Event e2 =
        cpus[i % cpus.size()].spawn(READER_TASK, &task_args, sizeof(ReaderTaskArgs), e1);
  ...
  // launch another writer task to compete reservation with reader tasks
  {
    ...
    Event e2 = cpus[TestConfig::num_readers % cpus.size()].spawn(
        WRITER_TASK1, &task_args, sizeof(WriterTaskArgs), e1);
    reservation.release(e2);
    task_events.push_back(e2);
  }
...
int main(int argc, char **argv)
{
  ...
  Processor::register_task_by_kind(Processor::LOC_PROC, false /*!global*/, MAIN_TASK,
      ...
      .wait();
  Processor::register_task_by_kind(Processor::LOC_PROC, false /*!global*/, WRITER_TASK1,
                                   CodeDescriptor(writer_task1), ProfilingRequestSet(), 0,
                                   0)
      .wait();
  Processor::register_task_by_kind(Processor::LOC_PROC, false /*!global*/, WRITER_TASK2,
                                   CodeDescriptor(writer_task2), ProfilingRequestSet(), 0,
                                   0)
      .wait();
  Processor::register_task_by_kind(Processor::LOC_PROC, false /*!global*/,
                                   WRITER_INNER_TASK, CodeDescriptor(writer_inner_task),
                                   ProfilingRequestSet(), 0, 0)
      .wait();
  Processor::register_task_by_kind(Processor::LOC_PROC, false /*!global*/, READER_TASK,
                                   CodeDescriptor(reader_task), ProfilingRequestSet(), 0,
                                   0)
      .wait();
