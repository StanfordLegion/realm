...
struct StructType {
  int value;
  bool operator!=(const StructType &other) const { return value != other.value; }
};
            ...
            size_t field_size, FID_T exp)
{
  ...
  if(proc.address_space() == dst_inst.get_location().address_space()) {
    ...
    Event e2 = RegionInstance::create_instance(
        inst, cpu_mem, domain, fields, 0 /*block_size=SOA*/, ProfilingRequestSet());
...
template <int N, typename T, typename FID_T>
Event reduce(RegionInstance src_inst, RegionInstance dst_inst, IndexSpace<N, T> domain,
             FieldID src_fid, FieldID dst_fid, size_t src_fsize, size_t dst_fsize,
             bool fold, bool exclusive, Event wait_on = Event::NO_EVENT)
{
  std::vector<CopySrcDstField> srcs(1), dsts(1);
...
bool do_reduction(IndexSpace<N, T> domain, IndexSpace<N, T> bloat, Memory dst_mem,
                  Memory src_mem)
{
  std::map<FieldID, size_t> fields;
  ...
  Event inst_event1 = RegionInstance::create_instance(dst_inst0, dst_mem, bloat, fields,
                                                      /*block_size=*/0,
                                                      ProfilingRequestSet(), start_event);

  Event inst_event2 = RegionInstance::create_instance(dst_inst1, dst_mem, bloat, fields,
                                                      /*block_size=*/0,
                                                      ProfilingRequestSet(), start_event);

  Event inst_event3 = RegionInstance::create_instance(src_inst, src_mem, bloat, fields,
                                                      /*block_size=*/0,
                                                      ProfilingRequestSet(), start_event);

  std::vector<Event> fill_events;
  fill_events.push_back(fill(src_inst, bloat, /*fid=*/FID_INT, /*val=*/1, inst_event1));
  fill_events.push_back(fill(dst_inst0, bloat, /*fid=*/FID_INT, /*val=*/3, inst_event2));
  ...
  Event apply_event1 = reduce<2, T, StructType>(dst_inst0, dst_inst1, domain,
                                                /*src_fid=*/FID_INT,
  ...
  Event apply_event2 = reduce<2, T, StructType>(dst_inst0, dst_inst1, domain,
                                                /*src_fid=*/FID_INT,
                                                /*dst_fid=*/FID_COMPLEX_TYPE,
                                                /*src_fsize=*/sizeof(int),
                                                /*dst_fsize=*/sizeof(StructType),
               ...
               Processor p)
{
  ...
  Machine::MemoryQuery mq =
      Machine::MemoryQuery(Machine::get_machine()).only_kind(Memory::Kind::SYSTEM_MEM);
  ...
  for(size_t i = 0; i < mems.size(); i++) {
    if(success) {
      success = do_reduction(IndexSpace<2>(Rect<2>(Point<2>(0, 0), Point<2>(3, 0))),
