...
void compute_prof_task(const void *args, size_t arglen, const void *userdata,
...
void copy_prof_task(const void *args, size_t arglen, const void *userdata, size_t userlen,
                    Processor p)
{
  ProfilingResponse resp(args, arglen);
  assert(resp.user_data_size() == sizeof(CopyProfResultWrapper));
  const CopyProfResultWrapper *result =
      static_cast<const CopyProfResultWrapper *>(resp.user_data());
  ...
  if(resp.get_measurement(copy_info)) {
    ...
    for(size_t i = 0; i < result->inst_size; i++) {
      ...
      if(metrics->inst_info[i].request_type ==
...
void inst_prof_task(const void *args, size_t arglen, const void *userdata, size_t userlen,
                    Processor p)
{
  ProfilingResponse resp(args, arglen);
  assert(resp.user_data_size() == sizeof(InstProfResultWrapper));
  const InstProfResultWrapper *result =
      static_cast<const InstProfResultWrapper *>(resp.user_data());
...
void compute_task(const void *args, size_t arglen, const void *userdata, size_t userlen,
                  Processor p)
{
...
int find_the_best_cpu(std::vector<std::pair<int, double>> &cpu_status,
                      std::vector<double> &batch_task_runtime, int task_idx,
                      bool use_profile)
{
  ...
  if(use_profile) {
    ...
    log_app.info("Decision: task %d, cpu %d, cpu runtime time %f", task_idx, idx,
                 lowest_cpu_runtime);
               ...
               Processor p)
{
  ...
  Processor profile_proc = Machine::ProcessorQuery(Machine::get_machine())
                               .local_address_space()
  ...
  for(Machine::ProcessorQuery::iterator it = pq.begin(); it; ++it) {
    // for better performance, we do not include the processor used by main_task for
    // worker tasks
    if((*it) != p) {
  ...
  task_prs
      .add_request(profile_proc, COMPUTE_PROF_TASK, &task_result,
                   sizeof(ComputeProfResultWrapper))
  ...
  worker_procs[0]
      .spawn(COMPUTE_TASK, &compute_task_args, sizeof(ComputeTaskArgs), task_prs)
      .wait();
  ...
  src_inst_prs
      .add_request(profile_proc, INST_PROF_TASK, &src_inst_result,
                   sizeof(InstProfResultWrapper))
      .add_measurement<ProfilingMeasurements::InstanceTimeline>()
      .add_measurement<ProfilingMeasurements::InstanceMemoryUsage>();
  RegionInstance::create_instance(src_inst, cpu_mem, ispace, field_sizes, 0, src_inst_prs)
      .wait();
  ...
  dst_inst_prs
      .add_request(profile_proc, INST_PROF_TASK, &dst_inst_result,
                   sizeof(InstProfResultWrapper))
      .add_measurement<ProfilingMeasurements::InstanceTimeline>()
      .add_measurement<ProfilingMeasurements::InstanceMemoryUsage>();
  RegionInstance::create_instance(dst_inst, cpu_mem, ispace, field_sizes, 0, dst_inst_prs)
      .wait();
  ...
  CopyProfResultWrapper fill_result;
  {
    ...
    fill_result.inst_size =
        2; // fill can only work on single field, so we need an array of two
    ProfilingRequestSet fill_prs;
    fill_prs
        .add_request(profile_proc, COPY_PROF_TASK, &fill_result,
                     sizeof(CopyProfResultWrapper))
        .add_measurement<ProfilingMeasurements::OperationTimeline>()
  ...
  CopyProfResultWrapper copy_result;
  {
    ...
    copy_prs
        .add_request(profile_proc, COPY_PROF_TASK, &copy_result,
                     sizeof(CopyProfResultWrapper))
        .add_measurement<ProfilingMeasurements::OperationTimeline>()
        .add_measurement<ProfilingMeasurements::OperationCopyInfo>()
        .add_measurement<ProfilingMeasurements::OperationMemoryUsage>();
  ...
  for(FieldID fid = FID_BASE; fid <= FID_BASE + 1; fid++) {
    ...
    for(IndexSpaceIterator<1, int> it(ispace); it.valid; it.step()) {
      for(PointInRectIterator<1, int> it2(it.rect); it2.valid; it2.step()) {
        ...
        if(act != expected) {
          log_app.error() << "mismatch: [" << it2.p << "] = " << act << " (expected "
                          << expected << ")";
        }
      }
    }
  ...
  log_app.print("Task on processor %llx ready at %lld, start at %lld, complete at %lld, "
                "total time %lld ns, wait time %lld ns",
                task_metrics.proc.id, task_metrics.ready_time, task_metrics.start_time,
                task_metrics.complete_time, task_total_time, task_wait_time);
  ...
  log_app.print(
      "Copy start at %lld, complete at %lld, total time %lld ns, "
      "src inst %llx on memory %llx, dst inst %llx on memory %llx, size %zu (B), num "
      "hops %u, src fid (%d, %d), dst fid (%d, %d)",
      copy_metrics.start_time, copy_metrics.complete_time, copy_total_time,
      copy_metrics.inst_info[0].src_insts[0].id, copy_metrics.src_mem.id,
      copy_metrics.inst_info[0].dst_insts[0].id, copy_metrics.dst_mem.id,
  ...
  src_inst_done.wait();
  log_app.print("Src instance on memory %llx, size %zu (B), created at %lld, ready at "
                "%lld, destroyed at %lld",
                src_inst_metrics.memory.id, src_inst_metrics.bytes,
                src_inst_metrics.create_time, src_inst_metrics.ready_time,
                src_inst_metrics.delete_time);

  dst_inst_done.wait();
  log_app.print("Dst instance on memory %llx, size %zu (B), created at %lld, ready at "
                "%lld, destroyed at %lld",
                dst_inst_metrics.memory.id, dst_inst_metrics.bytes,
                dst_inst_metrics.create_time, dst_inst_metrics.ready_time,
                dst_inst_metrics.delete_time);
  ...
  for(int i = 0; i < TestConfig::num_iterations; i++) {
    ...
    for(int j = 0; j < TestConfig::num_tasks; j++) {
      ...
      compute_task_args.cpu_idx =
          find_the_best_cpu(cpu_status, batch_task_runtime, j, use_profile);
      ...
      task_prs
          .add_request(profile_proc, COMPUTE_PROF_TASK, &task_result,
                       sizeof(ComputeProfResultWrapper))
          .add_measurement<ProfilingMeasurements::OperationTimeline>();
      Event e = worker_procs[compute_task_args.cpu_idx].spawn(
          COMPUTE_TASK, &compute_task_args, sizeof(ComputeTaskArgs), task_prs,
          start_event);
    ...
    for(int j = 0; j < TestConfig::num_tasks; j++) {
      prof_events[j].wait();
      double runtime =
          (batch_task_metrics[j].complete_time - batch_task_metrics[j].start_time) * 1e-9;
...
int main(int argc, char **argv)
{
  ...
  Processor::register_task_by_kind(Processor::LOC_PROC, false /*!global*/, COMPUTE_TASK,
                                   CodeDescriptor(compute_task), ProfilingRequestSet())
      .external_wait();

  Processor::register_task_by_kind(Processor::UTIL_PROC, false /*!global*/,
                                   COMPUTE_PROF_TASK, CodeDescriptor(compute_prof_task),
                                   ProfilingRequestSet())
      .external_wait();

  Processor::register_task_by_kind(Processor::UTIL_PROC, false /*!global*/,
                                   COPY_PROF_TASK, CodeDescriptor(copy_prof_task),
                                   ProfilingRequestSet())
      .external_wait();

  Processor::register_task_by_kind(Processor::UTIL_PROC, false /*!global*/,
                                   INST_PROF_TASK, CodeDescriptor(inst_prof_task),
                                   ProfilingRequestSet())
      .external_wait();
