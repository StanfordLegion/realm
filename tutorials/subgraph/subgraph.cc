...
void add_task(const void *args, size_t arglen, const void *userdata, size_t userlen,
              Processor p)
{
...
void init_task(const void *args, size_t arglen, const void *userdata, size_t userlen,
               Processor p)
{
...
void verify_task(const void *args, size_t arglen, const void *userdata, size_t userlen,
                 Processor p)
{
  ...
  for(IndexSpaceIterator<1> it(task_args.is); it.valid; it.step()) {
    for(PointInRectIterator<1> it2(it.rect); it2.valid; it2.step()) {
      if(acc[it2.p] != task_args.val) {
        log_app.error() << "MISMATCH: " << it2.p << ": " << acc[it2.p]
                        << " != " << task_args.val;
      }
    }
  }
               ...
               Processor p)
{
  ...
  Machine::ProcessorQuery pq = Machine::ProcessorQuery(Machine::get_machine())
                                   ...
                                   .only_kind(Processor::Kind::LOC_PROC);
  if(pq.count() < 3) {
    log_app.warning("The optimal number of CPU processor for this program is at least 3 "
                    "per rank, please specify it through -ll:cpu");
  }
  ...
  // create instances
  for(int i = 0; i < 5; i++) {
    RegionInstance::create_instance(insts[i], m, is, field_sizes, 0 /*block_size=SOA*/,
                                    ProfilingRequestSet())
        .wait();
  ...
  // first: without subgraph
  {
    ...
    log_app.print("Without subgraph, from constructing graph %.2f us, after constructing "
                  "graph %.2f us",
                  (t3 - t1) / 1e3, (t3 - t2) / 1e3);
  ...
  // launch the subgraph
  {
    ...
    for(int i = 0; i < TestConfig::num_iters; i++) {
      ...
      finish_events[i] =
          sg.instantiate(NULL, 0, ProfilingRequestSet(), preconds, postconds);
    ...
    e.wait();
    long long t3 = Clock::current_time_in_nanoseconds();
    log_app.print("With subgraph, from creating subgraph %.2f us, from instantiating "
                  "subgraph %.2f us, after instantiating subgraph %.2f us",
