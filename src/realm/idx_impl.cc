...
namespace Realm {
  ...
  void IndexSpaceGeneric::destroy(Event wait_on) { impl->destroy(wait_on); }
  ...
  Event IndexSpaceGeneric::copy(
      ...
      int priority /*= 0*/) const
  {
    return impl->copy(srcs, dsts, &indirects[0], indirects.size(), requests, wait_on,
                      priority);
  }
  ...
  template <int N, typename T>
  IndexSpaceGenericImplTyped<N, T>::IndexSpaceGenericImplTyped(
      const IndexSpace<N, T> &_space)
    : space(_space)
  {}
  ...
  template <int N, typename T>
  bool IndexSpaceGenericImplTyped<N, T>::compute_affine_bounds(
      const InstanceLayoutGeneric *ilg, FieldID fid, uintptr_t &rel_base,
      uintptr_t &limit) const
  {
    ...
    for(IndexSpaceIterator<N, T> isr(this->space); isr.valid; isr.step()) {
      for(typename std::vector<InstanceLayoutPiece<N, T> *>::const_iterator it2 =
        ...
        for(int i = 0; i < N; i++) {
          if(isect.lo[i] == isect.hi[i])
            continue; // trivial

          // not safe to directly substract isect.hi-isect.lo
          uintptr_t hi_ofs = (lo_ofs - (alp->strides[i] * isect.lo[i]) +
        ...
        if(!(*it2)->bounds.contains(isr.rect))
          break;
      }
    }
