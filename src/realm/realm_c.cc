...
static realm_status_t
realm_region_instance_create_common(Realm::RuntimeImpl *runtime_impl,
                                    Realm::InstanceLayoutGeneric *layout,
                                    realm_memory_t memory,
                                    const realm_external_resource_t *external_resource,
                                    const Realm::ProfilingRequestSet *prs_cxx,
                                    realm_event_t wait_on,
                                    Realm::RegionInstance &inst,
                                    Realm::Event &out_event)
{
  ...
  out_event = Realm::RegionInstanceImpl::create_instance(
    runtime_impl, inst, Realm::Memory(memory), layout,
    external_resource_cxx.get(), *prs_cxx, Realm::Event(wait_on));
    ...
    realm_region_instance_t *instance, realm_event_t *event)
{
  ...
  case REALM_COORD_TYPE_LONG_LONG:
  {
    ...
    status = realm_dim_dispatch<long long>(
        instance_creation_params->num_dims, RealmInstanceLayoutCreate(),
        lower_bound, upper_bound,
        instance_creation_params->field_ids,
        instance_creation_params->field_sizes,
        instance_creation_params->num_fields,
        instance_creation_params->block_size,
        instance_layout);
    break;
  }
  ...
  status = realm_region_instance_create_common(runtime_impl,
                                            instance_layout,
                                            instance_creation_params->memory,
                                            instance_creation_params->external_resource,
                                            prs_cxx,
                                            wait_on,
                                            inst,
                                            out_event);
...
realm_status_t realm_instance_layout_c_to_cxx(
  const realm_instance_layout_t *instance_layout,
  Realm::InstanceLayoutGeneric *&instance_layout_cxx)
{
  instance_layout_cxx = const_cast<Realm::InstanceLayoutGeneric *>(
      reinterpret_cast<const Realm::InstanceLayoutGeneric *>(instance_layout));
  return REALM_SUCCESS;
}
