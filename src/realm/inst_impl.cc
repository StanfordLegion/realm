...
namespace Realm {
                                                           ...
                                                           TimeLimit work_until)
  {
    ...
    mem->allocate_storage_immediate(inst, need_alloc_result, poisoned, work_until);
  }
  ...
  struct PieceSplitNode {
    ...
    template <int N>
    static PieceSplitNode<T> *
    compute_split(const std::vector<InstanceLayoutPiece<N, T> *> &pieces,
                  std::vector<int> &idxs)
    {
      ...
      while(true) {
        ...
        for(int dim = 0; dim < N; dim++) {
          ...
          for(size_t i = 0; i < idxs.size(); i++) {
            ss.push_back(std::make_pair(pieces[idxs[i]]->bounds.lo[dim], -1 - idxs[i]));
            ss.push_back(std::make_pair(pieces[idxs[i]]->bounds.hi[dim], idxs[i]));
          }
        ...
        if(best_dim >= 0) {
          ...
          n->total_splits =
              (1 + n->low_child->total_splits + n->high_child->total_splits);
          return n;
        } else {
    ...
    void print(const std::vector<InstanceLayoutPiece<N, T> *> &pieces, int indent)
    {
      for(size_t i = 0; i < presplit_pieces.size(); i++) {
        for(int j = 0; j < indent; j++)
          std::cout << ' ';
        std::cout << "piece[" << presplit_pieces[i]
                  << "]: " << pieces[presplit_pieces[i]]->bounds << "\n";
      }
      if(total_splits > 0) {
        for(int j = 0; j < indent; j++)
          std::cout << ' ';
    ...
    template <int N>
    char *generate_instructions(const std::vector<InstanceLayoutPiece<N, T> *> &pieces,
                                char *next_inst, unsigned &usage_mask)
    {
      ...
      for(size_t i = 0; i < presplit_pieces.size(); i++) {
        ...
        unsigned next_delta =
            (((i < (presplit_pieces.size() - 1)) || (total_splits > 0)) ? (bytes >> 4)
                                                                        : 0);
        PieceLookup::Instruction *inst =
            pieces[presplit_pieces[i]]->create_lookup_inst(next_inst, next_delta);
      ...
      if(total_splits > 0) {
        ...
        PieceLookup::SplitPlane<N, T> *sp = new(next_inst) PieceLookup::SplitPlane<N, T>(
            split_dim, split_plane, 0 /*we'll fix delta below*/);
        next_inst += size;
    ...
    for(std::map<FieldID, FieldLayout>::const_iterator it = fields.begin();
      ...
      if(pl.pieces.empty()) {
        ...
        for(size_t i = 0; i < pl.pieces.size(); i++)
          idxs[i] = i;
        PieceSplitNode<T> *plan = PieceSplitNode<T>::compute_split(pl.pieces, idxs);
        piece_list_plans[it->second.list_idx] = plan;
        // plan->print(pl.pieces, 2);
        total_bytes +=
            (plan->total_splits * roundup(sizeof(PieceLookup::SplitPlane<N, T>), 16));
      }
    }
  ...
  {
    return (exists()
                ? ID::make_memory(ID(id).instance_owner_node(), ID(id).instance_mem_idx())
                      .convert<Memory>()
                : Memory::NO_MEMORY);
  }
  ...
  /*static*/ Event RegionInstance::create_instance(RegionInstance &inst, Memory memory,
                                                   ...
                                                   Event wait_on)
  {
    return RegionInstanceImpl::create_instance(
        inst, get_runtime()->get_memory_impl(memory), ilg.clone(), 0, prs, wait_on);
  }
  ...
  {
    return RegionInstanceImpl::create_instance(
        inst, get_runtime()->get_memory_impl(memory), ilg, 0, prs, wait_on);
  }
  ...
  {
    return RegionInstanceImpl::create_instance(
        inst, get_runtime()->get_memory_impl(memory), ilg.clone(), &res, prs, wait_on);
  }
  ...
  RegionInstance::generate_resource_info(const IndexSpaceGeneric &space,
                                         span<const FieldID> fields, bool read_only) const
  {
    ID inst_id = ID(id);
    MemoryImpl *mem_impl = get_runtime()->get_memory_impl(inst_id);
    assert(mem_impl != nullptr && "invalid memory handle");
  ...
  RegionInstance::get_lookup_program(FieldID field_id, unsigned allowed_mask,
  ...
  template <int N, typename T>
  const PieceLookup::Instruction *
  RegionInstance::get_lookup_program(FieldID field_id, const Rect<N, T> &subrect,
  ...
  void RegionInstance::write_untyped(size_t offset, const void *data,
  ...
  RegionInstanceImpl::RegionInstanceImpl(const RuntimeImpl *_runtime_impl,
                                         RegionInstance _me, MemoryImpl *_memory)
    : me(_me)
    , mem_impl(_memory)
      ...
      const ExternalInstanceResource *res, const ProfilingRequestSet &prs, Event wait_on)
  {
    ...
    //  this point)
    Event ready_event;
    switch(mem_impl->allocate_storage_deferrable(impl, need_alloc_result, wait_on)) {
    ...
    if(res)
      log_inst.info() << "instance created: inst=" << inst << " external=" << *res
                      << " ready=" << ready_event;
    else
      log_inst.info() << "instance created: inst=" << inst << " bytes=" << ilg->bytes_used
                      << " ready=" << ready_event;
    return ready_event;
  }
                                       ...
                                       Event wait_on)
  {
    ...
    switch(mem_impl->reuse_storage_deferrable(this, insts, wait_on)) {
    case MemoryImpl::ALLOC_INSTANT_SUCCESS:
    {
      ready_event = Event::NO_EVENT;
      break;
                                             ...
                                             size_t offset, TimeLimit work_until)
  {
    ...
    if((result == MemoryImpl::ALLOC_INSTANT_FAILURE) ||
      ...
      if(!report_failure) {
        if((result == MemoryImpl::ALLOC_INSTANT_FAILURE) ||
           (result == MemoryImpl::ALLOC_EVENTUAL_FAILURE)) {
          log_inst.fatal() << "instance allocation failed - out of memory in mem "
                           << mem_impl->me;
          abort();
        }
                                                ...
                                                bool exclusive /*= false*/)
  {
    ...
    if(ptr) {
      ...
      mem_impl->put_bytes(metadata.inst_offset + offset, lhs_copy, redop->sizeof_lhs);
    }
  }
                                               ...
                                               bool exclusive /*= false*/)
  {
    ...
    if(ptr) {
      ...
      mem_impl->put_bytes(metadata.inst_offset + offset, rhs1_copy, redop->sizeof_rhs);
    }
  }
  ...
  /*static*/ void InstanceMetadataPrefetchRequest::handle_message(
      NodeID sender, const InstanceMetadataPrefetchRequest &msg, const void *data,
      size_t datalen, TimeLimit work_until)
  {
    ...
    log_inst.info() << "metadata prefetch: inst=" << msg.inst << " local=" << e
                    << " remote=" << msg.valid_event;
                                                  ...
                                                  off_t field_offset) const
  {
    ...
    if(stride != 0) {
      if((affine->bounds.hi[0] > affine->bounds.lo[0]) && (affine->strides[0] != stride))
        return false;
    } else {
    ...
    base = mem_impl->get_direct_ptr(start_offset, total_bytes);
    if(!base)
      return false;
  ...
  ExternalMemoryResource::ExternalMemoryResource(const void *_base, size_t _size_in_bytes)
    : ExternalInstanceResource(REALM_HASH_TOKEN(Realm::ExternalMemoryResource))
    , base(reinterpret_cast<uintptr_t>(_base))
    , size_in_bytes(_size_in_bytes)
    , read_only(true)
  ...
  /*static*/ Serialization::PolymorphicSerdezSubclass<ExternalInstanceResource,
      ...
      ExternalMemoryResource::serdez_subclass;
  ...
  /*static*/ Serialization::PolymorphicSerdezSubclass<ExternalInstanceResource,
      ...
      ExternalFileResource::serdez_subclass;

  template <int N, typename T>
  /*static*/ Serialization::PolymorphicSerdezSubclass<InstanceLayoutPiece<N, T>,
                                                      AffineLayoutPiece<N, T>>
      AffineLayoutPiece<N, T>::serdez_subclass;

  template <int N, typename T>
  /*static*/ Serialization::PolymorphicSerdezSubclass<InstanceLayoutGeneric,
                                                      InstanceLayout<N, T>>
      InstanceLayout<N, T>::serdez_subclass;

#define DOIT(N, T)                                                                       \
  template class AffineLayoutPiece<N, T>;                                                \
  template class InstanceLayout<N, T>;                                                   \
  template const PieceLookup::Instruction *RegionInstance::get_lookup_program<N, T>(     \
      FieldID, unsigned, uintptr_t &);                                                   \
  template const PieceLookup::Instruction *RegionInstance::get_lookup_program<N, T>(     \
      FieldID, const Rect<N, T> &, unsigned, uintptr_t &);
  FOREACH_NT(DOIT)
#undef DOIT
