...
namespace Realm {
  ...
  inline void CircularQueue<T, INTSIZE>::reserve(size_t new_capacity)
  {
    ...
    // easy case 1: queue is empty - just resize array and move head and tail back to
    // beginning
    if(current_size == 0) {
      if(external_buffer)
  ...
  {
    return *(item_ptr(external_buffer ? external_buffer : internal_buffer, head));
  }
  ...
  {
    return *(item_ptr(external_buffer ? external_buffer : internal_buffer, tail));
  }
  ...
  template <typename T, unsigned INTSIZE>
  inline typename CircularQueue<T, INTSIZE>::iterator
  CircularQueue<T, INTSIZE>::begin(void)
  {
  ...
  template <typename T, unsigned INTSIZE>
  inline typename CircularQueue<T, INTSIZE>::const_iterator
  CircularQueue<T, INTSIZE>::begin(void) const
  {
  ...
  template <typename T, unsigned INTSIZE>
  inline typename CircularQueue<T, INTSIZE>::const_iterator
  CircularQueue<T, INTSIZE>::end(void) const
  {
  ...
  template <typename T, unsigned INTSIZE>
  inline CircularQueueIterator<T, INTSIZE>::CircularQueueIterator(
      CircularQueue<T, INTSIZE> *_cq, size_t _pos, bool _at_end)
    : cq(_cq)
    , pos(_pos)
    , at_end(_at_end)
  {}
  ...
  template <typename T, unsigned INTSIZE>
  inline CircularQueueIterator<T, INTSIZE>::CircularQueueIterator(
      const CircularQueueIterator<T, INTSIZE> &copy_from)
    : cq(copy_from.cq)
    , pos(copy_from.pos)
  ...
  template <typename T, unsigned INTSIZE>
  inline CircularQueueIterator<T, INTSIZE> &CircularQueueIterator<T, INTSIZE>::operator=(
      const CircularQueueIterator<T, INTSIZE> &copy_from)
  {
    cq = copy_from.cq;
    pos = copy_from.pos;
  ...
  template <typename T, unsigned INTSIZE>
  inline bool CircularQueueIterator<T, INTSIZE>::operator==(
      const CircularQueueIterator<T, INTSIZE> &compare_to) const
  {
    // two at-end iterators always match
    if(at_end && compare_to.at_end)
      return true;
    // otherwise all fields must match
    return ((cq == compare_to.cq) && (pos == compare_to.pos) &&
