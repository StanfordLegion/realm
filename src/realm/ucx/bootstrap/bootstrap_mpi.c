...
MPI_Op mpi_ops[REDUCTION_LAST] = {[REDUCTION_SUM] = MPI_SUM};
                                ...
                                struct bootstrap_handle *handle)
{
  ...
  status = MPI_Gather(sendbuf, length, MPI_BYTE, recvbuf, length, MPI_BYTE, root,
                      bootstrap_comm);
  BOOTSTRAP_NE_ERROR_JMP(status, MPI_SUCCESS, BOOTSTRAP_ERROR_INTERNAL, out,
                         "MPI_Gather failed\n");
                                   ...
                                   struct bootstrap_handle *handle)
{
  ...
  status =
      MPI_Allgather(sendbuf, length, MPI_BYTE, recvbuf, length, MPI_BYTE, bootstrap_comm);
  BOOTSTRAP_NE_ERROR_JMP(status, MPI_SUCCESS, BOOTSTRAP_ERROR_INTERNAL, out,
                                  ...
                                  struct bootstrap_handle *handle)
{
  ...
  status =
      MPI_Alltoall(sendbuf, length, MPI_BYTE, recvbuf, length, MPI_BYTE, bootstrap_comm);
  BOOTSTRAP_NE_ERROR_JMP(status, MPI_SUCCESS, BOOTSTRAP_ERROR_INTERNAL, out,
                         "MPI_Alltoall failed\n");
                                       ...
                                       struct bootstrap_handle *handle)
{
  ...
  status = MPI_Allreduce(sendbuf, recvbuf, count, MPI_UNSIGNED_LONG_LONG, mpi_ops[op],
                         bootstrap_comm);
  BOOTSTRAP_NE_ERROR_JMP(status, MPI_SUCCESS, BOOTSTRAP_ERROR_INTERNAL, out,
...
static int populate_shared_ranks(bootstrap_handle_t *handle)
{
  ...
  for(size_t i = 0; i < mpi_proc_name_len; i++) {
    mpi_name_hash = (((mpi_name_hash << 5)) + mpi_name_hash) + (unsigned)mpi_proc_name[i];
  }
  ...
  status =
      MPI_Comm_split(bootstrap_comm, (int)mpi_name_hash, handle->pg_rank, &shared_comm);
  BOOTSTRAP_NE_ERROR_JMP(status, MPI_SUCCESS, BOOTSTRAP_ERROR_INTERNAL, out,
