...
namespace Realm {
  ...
  namespace Cuda {
                                     ...
                                     size_t max_xfer_fields, size_t &fields_total)
    {
      ...
      auto &rect = info.subrects[info.num_rects++];
      ...
      // ---------------------------------------------------------------------------
      // geometry of the first (potentially partial) dimension
      // ---------------------------------------------------------------------------
      const auto [in_ofs, out_ofs] = std::pair{in_cur.get_offset(), out_cur.get_offset()};
      const auto [in_dim, out_dim] = std::pair{in_cur.get_dim(), out_cur.get_dim()};
      size_t icount = in_cur.remaining(0);
      size_t ocount = out_cur.remaining(0);
      const size_t contig = min({icount, ocount, bytes_left});

      assert(in_dim && out_dim && "AddressList dim cannot be zero");

      rect.src.addr = in_base + in_ofs;
      rect.dst.addr = out_base + out_ofs;
      rect.extents[0] = contig;
      rect.extents[1] = rect.extents[2] = 1;
      rect.src.strides[0] = rect.dst.strides[0] = contig;
      rect.volume = contig;

      upd_align(rect.src.addr);
      upd_align(rect.dst.addr);
      upd_align(contig);

      attach_fields(in_cur, rect.src);
      attach_fields(out_cur, rect.dst);

      // ---------------------------------------------------------------------------
      // fast path – pure 1‑D
      // ---------------------------------------------------------------------------
      if((contig == bytes_left) || ((contig == icount) && (in_dim == 1)) ||
         ((contig == ocount) && (out_dim == 1))) {
        in_cur.advance(0, contig, max<size_t>(1, rect.src.num_fields));
        out_cur.advance(0, contig, max<size_t>(1, rect.dst.num_fields));
        return contig * fields_total;
      }

      // ---------------------------------------------------------------------------
      // grow to 2‑D  (id/od == sub‑dimension chosen for “lines’’)
      // ---------------------------------------------------------------------------
      int id = (contig < icount) ? 0 : 1;
      int od = (contig < ocount) ? 0 : 1;

      const auto stride_for = [](AddressListCursor &c, int d, size_t split_bytes) {
        return d == 0 ? split_bytes : c.get_stride(1);
      };

      auto in_lstride = stride_for(in_cur, id, contig);
      auto out_lstride = stride_for(out_cur, od, contig);

      if(contig < icount && (icount % contig))
        id = 1;
      if(contig < ocount && (ocount % contig))
        od = 1;

      icount = (id == 0) ? icount / contig : in_cur.remaining(id);
      ocount = (od == 0) ? ocount / contig : out_cur.remaining(od);
      const size_t lines = min({icount, ocount, bytes_left / contig});

      upd_align(in_lstride);
      upd_align(out_lstride);

      if((contig * lines == bytes_left) || ((lines == icount) && (id == in_dim - 1)) ||
         ((lines == ocount) && (od == out_dim - 1))) {
        rect.extents[1] = lines;
        rect.src.strides[0] = in_lstride;
        rect.src.strides[1] = lines;
        rect.dst.strides[0] = out_lstride;
        rect.dst.strides[1] = lines;
        rect.volume = contig * lines;

        in_cur.advance(id, lines * (id ? 1 : contig));
        out_cur.advance(od, lines * (od ? 1 : contig));
      ...
      // ---------------------------------------------------------------------------
      const auto pstride_for = [&](AddressListCursor &c, int &d, size_t lstride,
                                   size_t _lines, size_t &scale) {
        if(_lines < (d == 0 ? icount : ocount)) {
          scale *= _lines;
          return lstride * _lines;
        }
        ++d;
        scale = 1;
        return c.get_stride(d);
      };

      size_t iscale = (id == 0) ? contig * lines : lines;
      size_t oscale = (od == 0) ? contig * lines : lines;
      uintptr_t in_pstride = pstride_for(in_cur, id, in_lstride, lines, iscale);
      uintptr_t out_pstride = pstride_for(out_cur, od, out_lstride, lines, oscale);

      upd_align(in_pstride);
      upd_align(out_pstride);

      icount = in_cur.remaining(id);
      ocount = out_cur.remaining(od);
      const size_t planes = min({icount, ocount, bytes_left / (contig * lines)});

      const bool do_transpose = (in_lstride > in_pstride) || (out_lstride > out_pstride);
      if(do_transpose) {
        transpose.src = in_base + in_ofs;
        transpose.dst = out_base + out_ofs;
        transpose.src_strides[0] = in_lstride;
        transpose.src_strides[1] = in_pstride;
        transpose.dst_strides[0] = out_lstride;
        transpose.dst_strides[1] = out_pstride;
        transpose.extents[0] = contig;
        transpose.extents[1] = lines;
        transpose.extents[2] = planes;
        --info.num_rects; // remove the rect – handled by transpose
      } else {
        rect.extents[1] = lines;
        rect.extents[2] = planes;
        rect.src.strides[0] = in_lstride;
        rect.src.strides[1] = in_pstride / in_lstride;
        rect.dst.strides[0] = out_lstride;
        rect.dst.strides[1] = out_pstride / out_lstride;
        rect.volume = contig * lines * planes;
      }
