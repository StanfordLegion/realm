...
namespace Realm {
  ...
  inline TranslationTransform<N, T>::TranslationTransform(const Point<N, T> &_offset)
    : offset(_offset)
  {}
  ...
  inline Point<N, T>
  TranslationTransform<N, T>::operator[](const Point<N, T2> &point) const
  {
    return point + offset;
  }
  ...
  inline AffineTransform<M, N, T>::AffineTransform(const Matrix<M, N, T> &_transform,
  ...
  template <int M, int N, typename T>
  template <typename T2>
  inline Point<M, T> AffineTransform<M, N, T>::operator[](const Point<N, T2> &point) const
  {
  ...
  template <int N, typename T, int N2, typename T2>
  inline Point<N, T>
  StructuredTransform<N, T, N2, T2>::operator[](const Point<N2, T> &point) const
  {
  ...
  DomainTransform<N, T, N2, T2>::DomainTransform(
    ...
    , type(DomainTransformType::UNSTRUCTURED_PTR)
  {}
  ...
  {
    return IndexSpace<N, T>(*this).fill(dsts, requests, fill_value, fill_value_size,
                                        wait_on, priority);
  }
  ...
  inline Event Rect<N, T>::copy(const std::vector<CopySrcDstField> &srcs,
                                ...
                                int priority /*= 0*/) const
  {
    return IndexSpace<N, T>(*this).copy(srcs, dsts, requests, wait_on, priority);
  }

  template <int N, typename T>
  inline Event
  Rect<N, T>::copy(const std::vector<CopySrcDstField> &srcs,
                   const std::vector<CopySrcDstField> &dsts, const IndexSpace<N, T> &mask,
                   const ProfilingRequestSet &requests,
                   Event wait_on /*= Event::NO_EVENT*/, int priority /*= 0*/) const
  {
    return IndexSpace<N, T>(*this).copy(srcs, dsts, mask, requests, wait_on, priority);
  }
  ...
  template <int N, typename T>
  REALM_CUDA_HD inline IndexSpace<N, T>::IndexSpace(void)
  {}

  template <int N, typename T>
  REALM_CUDA_HD inline IndexSpace<N, T>::IndexSpace(const Rect<N, T> &_bounds)
    : bounds(_bounds)
  {
    sparsity.id = 0;
  }
  ...
  template <int N, typename T>
  REALM_CUDA_HD inline /*static*/ IndexSpace<N, T> IndexSpace<N, T>::make_empty(void)
  {
  ...
  template <int N, typename T>
  REALM_CUDA_HD inline bool IndexSpace<N, T>::empty(void) const
  {
    return bounds.empty();
  }
                                      ...
                                      int priority /*= 0*/) const
  {
    ...
    return copy(srcs, dsts, std::vector<const typename CopyIndirection<N, T>::Base *>(),
                                      ...
                                      int priority /*=0*/) const
  {
    return copy(std::move(srcs), std::move(dsts),
                std::vector<const typename CopyIndirection<N, T>::Base *>(), requests,
                wait_on, priority);
  }
  ...
  inline Event IndexSpace<N, T>::create_subspace_by_image(
      ...
      const IndexSpace<N, T> &image, const ProfilingRequestSet &reqs, Event wait_on) const
  {
    return create_subspaces_by_image(transform, {source}, {image}, reqs, wait_on);
  }
      ...
      Event wait_on) const
  {
    return create_subspaces_by_image_with_difference(
        DomainTransform<N, T, N2, T2>(field_data), sources, diff_rhs, images, reqs,
        wait_on);
  }
  ...
  Event IndexSpace<N, T>::create_subspaces_by_preimage(
      ...
      Event wait_on /*= Event::NO_EVENT*/) const
  {
    return create_subspaces_by_preimage(DomainTransform<N2, T2, N, T>(field_data),
