...
namespace PRealm {
  ...
  Event ThreadProfiler::NameClosure::find_instance_name(Realm::RegionInstance inst) const
  {
    for(std::vector<RegionInstance>::const_iterator it = instances.begin();
        it != instances.end(); it++)
      if(it->id == inst.id)
                                        ...
                                        Event critical)
  {
    ...
    Realm::ProfilingRequest &req =
        requests.add_request(current, Profiler::CALLBACK_TASK_ID, &args, sizeof(args),
                                        ...
                                        Event critical)
  {
    ...
    args.critical = critical;
    NameClosure *closure = new NameClosure;
    for(std::vector<CopySrcDstField>::const_iterator it = srcs.begin(); it != srcs.end();
        it++)
      closure->add_instance(it->inst);
    for(std::vector<CopySrcDstField>::const_iterator it = dsts.begin(); it != dsts.end();
        it++)
  ...
  Event ThreadProfiler::add_inst_request(ProfilingRequestSet &requests, Event critical)
  {
    Profiler &profiler = Profiler::get_profiler();
    if(!profiler.enabled)
  ...
  void ThreadProfiler::record_barrier_arrival(Event result, Event precondition)
  {
    ...
    if(!profiler.enabled || profiler.no_critical_paths)
      return;
    BarrierArrivalInfo &info = barrier_arrival_infos.emplace_back(BarrierArrivalInfo());
    info.performed = Realm::Clock::current_time_in_nanoseconds();
    info.result = result;
  ...
  void ThreadProfiler::record_event_merger(Event result, const Event *preconditions,
                                           size_t num_events)
  {
    if(!result.exists())
      return;
...
// Increment the number of pending callbacks we expect to see
  ...
  void ThreadProfiler::process_response(ProfilingResponse &response)
  {
    ...
    case COPY_PROF:
    {
      ...
      for(std::vector<InstInfo>::const_iterator it = cpinfo.inst_info.begin();
        ...
        if(it->src_indirection_inst.exists() || it->dst_indirection_inst.exists()) {
          ...
          if(it->src_indirection_inst.exists()) {
            ...
            indirect.src_inst_uid = closure->find_instance_name(it->src_indirection_inst);
          } else {
            indirect.src = 0;
            indirect.src_fid = 0;
          ...
          if(it->dst_indirection_inst.exists()) {
            ...
            indirect.dst_inst_uid = closure->find_instance_name(it->dst_indirection_inst);
          } else {
            indirect.dst = 0;
            indirect.dst_fid = 0;
    ...
    case INST_PROF:
    {
      ...
      InstTimelineInfo &info = inst_timeline_infos.emplace_back(InstTimelineInfo());
  ...
  Profiler::Profiler(void)
    ...
    , implicit_proc(Processor::NO_PROC)
    , output_footprint_threshold(128 << 20 /*128MB*/)
    , target_latency(100 /*us*/)
    , total_memory_footprint(0)
    , shutdown_wait(Realm::UserEvent::NO_USER_EVENT)
    , return_code(0)
    , has_shutdown(false)
    , enabled(false)
    , self_profile(false)
  ...
  void Profiler::log_preamble(void) const
  {
    ...
    ss << "ZeroTime {"
       << "id:" << ZERO_TIME_ID << delim << "zero_time:long long:" << sizeof(long long)
       << "}" << std::endl;

    ss << "MemDesc {"
       << "id:" << MEM_DESC_ID << delim << "mem_id:MemID:" << sizeof(MemID) << delim
       << "kind:MemKind:" << sizeof(MemKind) << delim
       << "capacity:unsigned long long:" << sizeof(unsigned long long) << "}"
       << std::endl;

    ss << "ProcMDesc {"
       << "id:" << PROC_MEM_DESC_ID << delim << "proc_id:ProcID:" << sizeof(ProcID)
       << delim << "mem_id:MemID:" << sizeof(MemID) << delim
    ...
    ss << "TaskVariant {"
       ...
       << delim << "variant_id:VariantID:" << sizeof(VariantID) << delim << "name:string:"
       << "-1"
       << "}" << std::endl;

    ss << "TaskWaitInfo {"
       << "id:" << TASK_WAIT_INFO_ID << delim << "op_id:UniqueID:" << sizeof(UniqueID)
    ...
    ss << "TaskInfo {"
       << "id:" << TASK_INFO_ID << delim << "op_id:UniqueID:" << sizeof(UniqueID) << delim
    ...
    ss << "ImplicitTaskInfo {"
       << "id:" << IMPLICIT_TASK_INFO_ID << delim << "op_id:UniqueID:" << sizeof(UniqueID)
       << delim << "task_id:TaskID:" << sizeof(TaskID) << delim
       << "variant_id:VariantID:" << sizeof(VariantID) << delim
       << "proc_id:ProcID:" << sizeof(ProcID) << delim
       << "create:timestamp_t:" << sizeof(timestamp_t) << delim
    ...
    ss << "GPUTaskInfo {"
       << "id:" << GPU_TASK_INFO_ID << delim << "op_id:UniqueID:" << sizeof(UniqueID)
       << delim << "task_id:TaskID:" << sizeof(TaskID) << delim
       << "variant_id:VariantID:" << sizeof(VariantID) << delim
       << "proc_id:ProcID:" << sizeof(ProcID) << delim
       << "create:timestamp_t:" << sizeof(timestamp_t) << delim
    ...
    ss << "CopyInfo {"
       << "id:" << COPY_INFO_ID << delim << "op_id:UniqueID:" << sizeof(UniqueID) << delim
    ...
    ss << "CopyInstInfo {"
       ...
       << "dst:MemID:" << sizeof(MemID) << delim << "src_fid:unsigned:" << sizeof(FieldID)
    ...
    ss << "FillInfo {"
       << "id:" << FILL_INFO_ID << delim << "op_id:UniqueID:" << sizeof(UniqueID) << delim
       << "size:unsigned long long:" << sizeof(unsigned long long) << delim
       << "create:timestamp_t:" << sizeof(timestamp_t) << delim
       << "ready:timestamp_t:" << sizeof(timestamp_t) << delim
       << "start:timestamp_t:" << sizeof(timestamp_t) << delim
    ...
    ss << "InstTimelineInfo {"
       ...
       << "inst_uid:unsigned long long:" << sizeof(Event) << delim
       << "inst_id:InstID:" << sizeof(InstID) << delim << "mem_id:MemID:" << sizeof(MemID)
       << delim << "size:unsigned long long:" << sizeof(unsigned long long) << delim
       << "op_id:UniqueID:" << sizeof(UniqueID) << delim
       << "create:timestamp_t:" << sizeof(timestamp_t) << delim
       << "ready:timestamp_t:" << sizeof(timestamp_t) << delim
    ...
    ss << "PartitionInfo {"
       << "id:" << PARTITION_INFO_ID << delim << "op_id:UniqueID:" << sizeof(UniqueID)
       << delim << "part_op:DepPartOpKind:" << sizeof(DepPartOpKind) << delim
       << "create:timestamp_t:" << sizeof(timestamp_t) << delim
       << "ready:timestamp_t:" << sizeof(timestamp_t) << delim
       << "start:timestamp_t:" << sizeof(timestamp_t) << delim
       << "stop:timestamp_t:" << sizeof(timestamp_t) << delim
    ...
    ss << "ReservationAcquireInfo {"
       ...
       << "precondition:unsigned long long:" << sizeof(Event) << delim
       << "performed:timestamp_t:" << sizeof(timestamp_t) << delim
       << "reservation:unsigned long long:" << sizeof(Reservation) << "}" << std::endl;
    ...
    ss << "ProfTaskInfo {"
       << "id:" << PROFTASK_INFO_ID << delim << "proc_id:ProcID:" << sizeof(ProcID)
       << delim << "op_id:UniqueID:" << sizeof(UniqueID) << delim
       << "start:timestamp_t:" << sizeof(timestamp_t) << delim
       << "stop:timestamp_t:" << sizeof(timestamp_t) << delim
       << "creator:unsigned long long:" << sizeof(Event) << delim
       << "fevent:unsigned long long:" << sizeof(Event) << delim
  ...
  void Profiler::serialize(const ThreadProfiler::ReservationAcquireInfo &info) const
  {
  ...
  void Profiler::serialize(const ThreadProfiler::TaskInfo &task_info) const
  {
    ...
    pr_fwrite(f, (char *)&(task_info.finish_event), sizeof(task_info.finish_event));
  ...
  void
  Profiler::serialize(const ThreadProfiler::InstTimelineInfo &inst_timeline_info) const
  {
    ...
    pr_fwrite(f, (char *)&(inst_timeline_info.mem_id), sizeof(inst_timeline_info.mem_id));
    pr_fwrite(f, (char *)&(inst_timeline_info.size), sizeof(inst_timeline_info.size));
    unsigned long long op_id = inst_timeline_info.creator.id;
    pr_fwrite(f, (char *)&op_id, sizeof(op_id));
    pr_fwrite(f, (char *)&(inst_timeline_info.create), sizeof(inst_timeline_info.create));
    pr_fwrite(f, (char *)&(inst_timeline_info.ready), sizeof(inst_timeline_info.ready));
  ...
  void Profiler::serialize(const ThreadProfiler::ProfTaskInfo &proftask_info) const
  {
...
#ifdef DEBUG_REALM
  void Profiler::increment_total_outstanding_requests(ThreadProfiler::ProfKind kind)
  {
    AutoLock p_lock(profiler_lock);
    total_outstanding_requests[kind]++;
  }

  void Profiler::decrement_total_outstanding_requests(ThreadProfiler::ProfKind kind)
  {
    AutoLock p_lock(profiler_lock);
  ...
  Processor Profiler::get_implicit_processor(void)
  {
    ...
    if(!implicit_proc.exists()) {
      ...
      assert(count > 0);
      implicit_proc.id = Realm::ID::make_processor(local_proc.address_space(), count).id;
...
#undef PROC_NAMES
  ...
  void Profiler::record_processor(const Processor &p)
  {
    ...
    for(std::vector<Machine::ProcessorMemoryAffinity>::const_iterator pit =
      ...
      if(!std::binary_search(recorded_memories.begin(), recorded_memories.end(), pit->m))
  ...
  void
  Profiler::record_implicit(long long start_time, Realm::Event fevent,
                            const std::vector<ThreadProfiler::WaitInfo> &wait_intervals)
  {
    ...
    ID = TASK_WAIT_INFO_ID;
    for(std::vector<ThreadProfiler::WaitInfo>::const_iterator it = wait_intervals.begin();
  ...
  {
    AutoLock p_lock(profiler_lock);
    while(!remote_notifications.empty() && remote_notifications.front().has_triggered())
                                    ...
                                    Realm::Processor p)
  {
    ...
    if(wargs->after.exists()) {
      ...
      const ThreadProfiler::EventTriggerInfo info{wargs->after, wargs->fevent, done,
                                                  wargs->spawn_time};
      const Realm::Event notified =
  ...
  /*static*/ void Profiler::external(const void *args, size_t arglen,
                                     const void *user_args, size_t user_arglen,
                                     Realm::Processor p)
  {
    Realm::ProfilingResponse response(args, arglen);
  ...
  Event Processor::register_task(TaskFuncID task_id, const CodeDescriptor &codedesc,
                                 const ProfilingRequestSet &prs, const void *user_data,
  ...
  /*static*/ Event Processor::register_task_by_kind(
      Kind kind, bool global, TaskFuncID task_id, const CodeDescriptor &codedesc,
      const ProfilingRequestSet &prs, const void *user_data, size_t user_data_len)
  {
    Profiler &profiler = Profiler::get_profiler();
    profiler.record_task(task_id, std::string_view());
    ...
    return Realm::Processor::register_task_by_kind(kind, global, task_id, codedesc, prs,
    ...
    if(profiler.local_proc.address_space() != address_space()) {
      ...
      if(requests.empty()) {
        ...
        if(arglen > 0) {
          ...
          Realm::Processor::spawn(Profiler::WRAPPER_TASK_ID, wrapper_args, buffer_size,
  ...
  Event Runtime::collective_spawn(Processor target_proc, Processor::TaskFuncID task_id,
                                  const void *args, size_t arglen, Event wait_on,
                                  int priority)
  {
    ...
    if(arglen > 0) {
      ...
      std::memcpy(wrapper_args + 1, args, arglen);
      const Event result =
          Realm::Runtime::collective_spawn(target_proc, Profiler::WRAPPER_TASK_ID,
                                           wrapper_args, buffer_size, wait_on, priority);
      std::free(wrapper_args);
      return result;
    } else {
      ...
      return Realm::Runtime::collective_spawn(target_proc, Profiler::WRAPPER_TASK_ID,
                                              &wrapper_args, sizeof(wrapper_args),
                                              wait_on, priority);
    }
  }
  ...
  Event Runtime::collective_spawn_by_kind(Processor::Kind target_kind,
                                          ...
                                          size_t arglen, bool one_per_node, Event wait_on,
                                          int priority)
  {
    ...
    if(arglen > 0) {
      ...
      const Event result = Realm::Runtime::collective_spawn_by_kind(
          target_kind, Profiler::WRAPPER_TASK_ID, wrapper_args, buffer_size, one_per_node,
          wait_on, priority);
      std::free(wrapper_args);
      return result;
    } else {
      ...
      return Realm::Runtime::collective_spawn_by_kind(
          target_kind, Profiler::WRAPPER_TASK_ID, &wrapper_args, sizeof(wrapper_args),
  ...
  /*static*/ Runtime Runtime::get_runtime(void) { return Realm::Runtime::get_runtime(); }
