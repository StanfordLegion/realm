...
namespace PRealm {
  ...
  /*static*/ inline Event Event::merge_events(Event ev1, Event ev2, Event ev3, Event ev4,
                                              Event ev5, Event ev6)
  {
    size_t num_events;
    if(ev6.exists())
  ...
  /*static*/ inline Event Event::merge_events(const std::set<Event> &wait_for)
  {
    ...
    ThreadProfiler::get_thread_profiler().record_event_merger(result, &events.front(),
                                                              events.size());
    return result;
  }
  ...
  /*static*/ inline Event Event::merge_events(const span<const Event> &wait_for)
  {
    ...
    ThreadProfiler::get_thread_profiler().record_event_merger(result, wait_for.data(),
                                                              wait_for.size());
    return result;
  }
  ...
  inline Event Reservation::acquire(unsigned mode, bool exclusive, Event wait_on) const
  {
    ...
    ThreadProfiler::get_thread_profiler().record_reservation_acquire(*this, result,
                                                                     wait_on);
    return result;
  }
  ...
  /*static*/ inline ProcessorGroup ProcessorGroup::create_group(const Processor *members,
                                                                size_t num_members)
  {
    return ProcessorGroup(Realm::ProcessorGroup::create_group(members, num_members));
  }

  /*static*/ inline ProcessorGroup
  ProcessorGroup::create_group(const span<const Processor> &members)
  {
  ...
  /*static*/ inline Event
  RegionInstance::create_instance(RegionInstance &inst, Memory memory,
                                  ...
                                  const ProfilingRequestSet &requests, Event wait_on)
  {
    ...
    Event result =
        Realm::RegionInstance::create_instance(inst, memory, ilg, alt_requests, wait_on);
  ...
  /*static*/ inline Event RegionInstance::create_external_instance(
      ...
      const ExternalInstanceResource &resource, const ProfilingRequestSet &requests,
      Event wait_on)
  {
    ProfilingRequestSet alt_requests = requests;
    ThreadProfiler &profiler = ThreadProfiler::get_thread_profiler();
  ...
  /*static*/ inline Event
  RegionInstance::create_external(RegionInstance &inst, Memory memory, uintptr_t base,
                                  InstanceLayoutGeneric *ilg,
                                  const ProfilingRequestSet &requests, Event wait_on)
  {
  ...
  /*static*/ inline Event RegionInstance::create_file_instance(
      ...
      realm_file_mode_t file_mode, const ProfilingRequestSet &prs, Event wait_on)
  {
                                ...
                                Event wait_on, int priority) const
  {
    ...
    return Realm::Rect<N, T>::fill(alt_dsts, alt_requests, fill_value, fill_value_size,
                                   wait_on, priority);
  }
  ...
  {
    ProfilingRequestSet alt_requests = requests;
    ThreadProfiler::get_thread_profiler().add_copy_request(alt_requests, srcs, dsts,
                                                           wait_on);
                                      ...
                                      int priority) const
  {
    ...
    for(unsigned idx = 0; idx < dsts.size(); idx++)
      alt_dsts[idx] = dsts[idx];
    return Realm::IndexSpace<N, T>::copy(alt_srcs, alt_dsts, alt_requests, wait_on,
                                         priority);
  }
      ...
      const ProfilingRequestSet &requests, Event wait_on, int priority) const
  {
    ...
    for(unsigned idx = 0; idx < dsts.size(); idx++)
      alt_dsts[idx] = dsts[idx];
    return Realm::IndexSpace<N, T>::copy(alt_srcs, alt_dsts, indirects, alt_requests,
                                         wait_on, priority);
  }
                                       ...
                                       int priority) const
  {
    ...
    for(unsigned idx = 0; idx < dsts.size(); idx++)
      alt_dsts[idx] = dsts[idx];
    return Realm::IndexSpaceGeneric::copy(alt_srcs, alt_dsts, alt_requests, wait_on,
                                          priority);
  }
