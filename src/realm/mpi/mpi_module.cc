/*
 * Copyright 2025 Stanford University, NVIDIA Corporation, Argonne National Laboratory
...
void enqueue_message(int target, int msgid, const void *args, size_t arg_size,
                     const void *payload, size_t payload_size, size_t payload_lines,
                     size_t payload_line_stride, MPI_Aint dst_offset, void *remote_comp)
{
  Realm::MPI::AMSend(target, msgid, arg_size, payload_size, (const char *)args,
                     (const char *)payload, payload_lines, payload_line_stride,
                     (dst_offset >= 0), dst_offset, remote_comp);
}
...
namespace Realm {
   ...
   * To spread the memory access evenly, it uses round-robin policy with "memory_stride"
   * chunk size. NOTE: the MPI window is assumed to be opened with MPI_Win_unlock_all and
   * to be freed by "user"
   */
  ...
  public:
    ...
    MPIMemory(RuntimeImpl *_runtime_impl, Memory _me, size_t size_per_node, MPI_Win _win,
  ...
  void MPIMemory::get_bytes(off_t offset, void *dst, size_t size)
  {
    ...
    while(size > 0) {
      ...
      CHECK_MPI(
          MPI_Get(dst_c, chunk_size, MPI_BYTE, node, disp, chunk_size, MPI_BYTE, win));
  ...
  void MPIMemory::put_bytes(off_t offset, const void *src, size_t size)
  {
    ...
    while(size > 0) {
      ...
      MPI_Aint disp = blkid * memory_stride + blkoffset;
      CHECK_MPI(
          MPI_Put(src_c, chunk_size, MPI_BYTE, node, disp, chunk_size, MPI_BYTE, win));
  ...
  void MPIMemory::get_batch(size_t batch_size, const off_t *offsets, void *const *dsts,
                            const size_t *sizes)
  {
  ...
  void MPIMemory::put_batch(size_t batch_size, const off_t *offsets,
  ...
  public:
    MPIRemoteMemory(RuntimeImpl *_runtime_impl, Memory _me, size_t _size, Memory::Kind k,
  ...
  public:
    MPIMessageImpl(NodeID _target, unsigned short _msgid, size_t _header_size,
  ...
  MPIMessageImpl::MPIMessageImpl(NodeID _target, unsigned short _msgid,
                                 size_t _header_size, size_t _max_payload_size,
                                 const void *_src_payload_addr, size_t _src_payload_lines,
                                 size_t _src_payload_line_stride)
    : target(_target)
  ...
  void MPI::AMComplete(void *comp)
  {
    ...
    CompletionCallbackBase::invoke_all(remote_comp->storage, remote_comp->bytes);
    CompletionCallbackBase::destroy_all(remote_comp->storage, remote_comp->bytes);
    delete remote_comp;
  }
  ...
  public:
    ...
    {
      core_rsrv = new Realm::CoreReservation("AM workers", crs,
    ...
    void start_thread()
    {
      ...
      p_thread =
          Realm::Thread::create_kernel_thread<AM_Manager, &AM_Manager::thread_loop>(
              this, tlp, *core_rsrv);
    }
  ...
  */
  ...
  MPIModule::create_network_module(RuntimeImpl *runtime, int *argc, const char ***argv)
  {
  ...
  void MPIModule::get_shared_peers(NodeSet &shared_peers)
  {
    ...
    CHECK_MPI(MPI_Comm_split_type(MPI_COMM_WORLD, MPI_COMM_TYPE_SHARED, 0, MPI_INFO_NULL,
                                  &shared_comm));
    ...
    for(int i = 0; i < mpi_proc_name_len; i++) {
      mpi_name_hash = (((mpi_name_hash << 5)) + mpi_name_hash) +
                      static_cast<unsigned>(mpi_proc_name[i]);
    }
    CHECK_MPI(MPI_Allgather(&mpi_name_hash, 1, MPI_UNSIGNED, all_hashes.data(), 1,
  ...
  void MPIModule::attach(RuntimeImpl *runtime, std::vector<NetworkSegment *> &segments)
  {
    ...
    void *baseptr;
    CHECK_MPI(MPI_Win_allocate(attach_size, 1, MPI_INFO_NULL, MPI_COMM_WORLD, &baseptr,
                               &g_am_win));
    CHECK_MPI(MPI_Win_lock_all(0, g_am_win));

    Realm::MPI::AM_init_long_messages(g_am_win, baseptr, runtime->message_manager);

    int num_nodes = Network::max_node_id + 1;
    int size_p = sizeof(baseptr);
    g_am_bases = (void **)malloc(size_p * num_nodes);
    CHECK_MPI(MPI_Allgather(&baseptr, size_p, MPI_BYTE, g_am_bases, size_p, MPI_BYTE,
                            MPI_COMM_WORLD));
  ...
  void MPIModule::detach(RuntimeImpl *runtime, std::vector<NetworkSegment *> &segments)
  {
  ...
  void MPIModule::barrier(void) { CHECK_MPI(MPI_Barrier(MPI_COMM_WORLD)); }
  ...
  {
    CHECK_MPI(MPI_Gather(val_in, bytes, MPI_BYTE, vals_out, bytes, MPI_BYTE, root,
                         MPI_COMM_WORLD));
  }
      ...
      size_t src_payload_line_stride, void *storage_base, size_t storage_size)
  {
    ...
    MPIMessageImpl *impl = new(storage_base)
        MPIMessageImpl(target, msgid, header_size, max_payload_size, src_payload_addr,
  ...
  size_t MPIModule::recommended_max_payload(NodeID target, const void *data,
