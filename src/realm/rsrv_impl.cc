...
namespace Realm {
  ...
  public:
    ...
    virtual void event_triggered(bool poisoned, TimeLimit work_until)
    {
      ...
      if(poisoned) {
        log_poison.info() << "poisoned deferred lock skipped - lock=" << lock
                          << " after=" << after_lock;
        GenEventImpl::trigger(after_lock, true /*poisoned*/, work_until);
      } else {
        get_runtime()->get_lock_impl(lock)->acquire(
            mode, exclusive, ReservationImpl::ACQUIRE_BLOCKING, after_lock);
      }
  ...
  public:
    ...
    virtual void event_triggered(bool poisoned, TimeLimit work_until)
    {
      ...
      //  paying attention
      if(poisoned) {
        log_poison.warning() << "poisoned deferred unlock skipped - POSSIBLE HANG - lock="
                             << lock;
      } else {
    ...
    virtual void print(std::ostream &os) const { os << "deferred unlock: lock=" << lock; }
  ...
  public:
    DeferredLockDestruction(Reservation _lock)
      : lock(_lock)
    {}
    ...
    virtual void event_triggered(bool poisoned, TimeLimit work_until)
    {
      ...
      if(poisoned) {
        log_poison.info()
            << "poisoned deferred lock destruction skipped - POSSIBLE LEAK - lock="
            << lock;
      } else {
  ...
  /*static*/ const Reservation Reservation::NO_RESERVATION{REALM_NO_RESERVATION
                                                           /* zero-initialization */};
                             ...
                             Event wait_on /* = Event::NO_EVENT */) const
  {
    ...
    if(wait_on.has_triggered_faultaware(poisoned)) {
      ...
      EventImpl::add_waiter(wait_on,
                            new DeferredLockRequest(*this, mode, exclusive, after_lock));
      // printf("*(" IDFMT "/%d)\n", after_lock.id, after_lock.gen);
      return after_lock;
    }
  }

  Event Reservation::try_acquire(bool retry, unsigned mode /* = 0 */,
                                 bool exclusive /* = true */,
                                 Event wait_on /* = Event::NO_EVENT */) const
  {
    ReservationImpl *impl = get_runtime()->get_lock_impl(*this);

    // if we have an unsatisfied precondition, we need to wait for that before actually
    // trying
    //  to get the reservation, but those will be marked as retries, so get a placeholder
    //  request in here
    if(!wait_on.has_triggered()) {
      impl->acquire(mode, exclusive, ReservationImpl::ACQUIRE_NONBLOCKING_PLACEHOLDER);
      log_reservation.info() << "reservation try_acquire: rsrv=" << *this
                             << " wait_on=" << wait_on << " finish=" << wait_on;
      return wait_on;
    }

    // attempt the nonblocking acquire
    Event e = impl->acquire(mode, exclusive,
                            (retry ? ReservationImpl::ACQUIRE_NONBLOCKING_RETRY
                                   : ReservationImpl::ACQUIRE_NONBLOCKING));
    log_reservation.info() << "reservation try_acquire: rsrv=" << *this
                           << " wait_on=" << wait_on << " finish=" << e;
    return e;
  }
  ...
  /*static*/ void LockReleaseMessage::handle_message(NodeID sender,
                                                     const LockReleaseMessage &msg,
                                                     const void *data, size_t datalen)
  {
    assert(0);
  }

  /*static*/ void LockGrantMessage::handle_message(NodeID sender,
                                                   const LockGrantMessage &args,
                                                   const void *data, size_t datalen,
                                                   TimeLimit work_until)
  {
    log_reservation.debug("reservation request granted: reservation=" IDFMT
                          " mode=%d",               // mask=%lx",
    ...
    if(!to_wake.empty())
      get_runtime()->event_triggerer.trigger_event_waiters(to_wake, false /*!poisoned*/,
                                                           work_until);
                                 ...
                                 Event after_lock /*= Event:NO_EVENT*/)
  {
    log_reservation.debug() << "local reservation request: reservation=" << me
                            ...
                            << " acq=" << acquire_type << " event=" << after_lock
...

    {
      ...
      assert((NodeID(ID(me).rsrv_creator_node()) != Network::my_node_id) || in_use);
      ...
      if(owner == Network::my_node_id) {
        ...
        if((count == ZERO_COUNT) ||
           ((mode == new_mode) && (mode != MODE_EXCL) && local_excl_waiters.empty() &&
            (local_shared.empty() || local_shared.begin()->first > mode))) {
          mode = new_mode;
          count++;
          log_reservation.spew("count ++(1) [%p]=%d", static_cast<void *>(&count), count);
          got_lock = true;
#ifdef DEBUG_REALM
        ...
        if(!got_lock && !requested) {
          log_reservation.debug("requesting reservation: reservation=" IDFMT
                                " node=%d mode=%d",
      ...
      log_reservation.debug("local reservation result: reservation=" IDFMT
                            " got=%d req=%d count=%d",
                            me.id, got_lock ? 1 : 0, requested ? 1 : 0, count);

      // if this was a successful retry of a nonblocking request, decrement the
      // retry_count
  ...
  bool ReservationImpl::select_local_waiters(WaiterList &to_wake, Event &retry)
  {
  ...
  void ReservationImpl::release(TimeLimit work_until)
  {
    ...
    do {
      ...
      log_reservation.debug("release: reservation=" IDFMT
                            " count=%d mode=%d owner=%d", // share=%lx wait=%lx",
                            me.id, count, mode,
                            owner); //, remote_sharer_mask, remote_waiter_mask);
#endif
      ...
      // case 3: we can grant to a remote waiter (if any) if we don't expect any local
      // retries
    ...
    if(release_target != -1) {
      log_reservation.debug("releasing reservation " IDFMT " back to owner %d", me.id,
                            release_target);
  ...
  bool ReservationImpl::is_locked(unsigned check_mode, bool excl_ok)
  {
    ...
    bool held;
    {
      ...
      held = ((count > ZERO_COUNT) && ((mode == check_mode) || ((mode == 0) && excl_ok)));
    }
  ...
  /*static*/ void DestroyLockMessage::handle_message(NodeID sender,
                                                     const DestroyLockMessage &args,
                                                     const void *data, size_t datalen)
  {
  ...
  Event FastRsrvState::request_base_rsrv(FastReservation &frsv)
  {
    ...
    if(!rsrv_ready.exists())
      rsrv_ready =
          rsrv_impl->acquire(0, true /*excl*/, ReservationImpl::ACQUIRE_BLOCKING);
    ...
    if(rsrv_ready.has_triggered()) {
      ...
      FastReservation::State prev =
          frsv.state.fetch_sub(FastReservation::STATE_BASE_RSRV);
  ...
  namespace ThreadLocal {
    thread_local FastReservationDebugInfo *frsv_debug = 0;
  };
#endif
  ...
  FastReservation::FastReservation(Reservation _rsrv /*= Reservation::NO_RESERVATION*/)
  {
    if(sizeof(FastRsrvState) > sizeof(opaque)) {
      log_reservation.fatal() << "FastReservation opaque state too small! ("
                              << sizeof(opaque) << " < " << sizeof(FastRsrvState);
      assert(0);
    }
  ...
  // NOT copyable
  FastReservation::FastReservation(const FastReservation &) { assert(0); }
  ...
  Event FastReservation::wrlock_slow(WaitMode mode)
  {
    ...
    if((state.load() & STATE_SLOW_FALLBACK) != 0) {
      ...
      } while(!fallback_retry_count.compare_exchange(current_count, current_count - 1));
    ...
    while(1) {
      ...
      if((cur_state & (STATE_SLOW_FALLBACK | STATE_BASE_RSRV | STATE_BASE_RSRV_WAITING |
        ...
        if((mode == SPIN) || (mode == ALWAYS_SPIN)) {
          ...
          state.compare_exchange(cur_state, cur_state | STATE_WRITER_WAITING);
      ...
      // any other transition requires holding the fast reservation's mutex
      {
        ...
        while(true) {
          ...
          log_reservation.fatal()
              << "wrlock_slow: unexpected state = " << std::hex << cur_state << std::dec;
          assert(0);
        }
  ...
  Event FastReservation::rdlock_slow(WaitMode mode)
  {
    ...
    while(1) {
      ...
      if(((cur_state &
           (STATE_SLOW_FALLBACK | STATE_BASE_RSRV | STATE_BASE_RSRV_WAITING)) == 0) &&
         !sleeping_writer) {
      ...
      // any other transition requires holding the fast reservation's mutex
      {
        ...
        while(true) {
          ...
          // other cases?
          log_reservation.fatal()
              << "rdlock_slow: unexpected state = " << std::hex << cur_state << std::dec;
          assert(0);
        }
  ...
  void FastReservation::advise_sleep_entry(UserEvent guard_event)
  {
    ...
    if(frs.sleeper_count == 0) {
      ...
      if(guard_event != frs.sleeper_event)
        frs.sleeper_event = Event::merge_events(frs.sleeper_event, guard_event);
    }
    frs.mutex.unlock();
  }
                                          ...
                                          const void *data, size_t datalen)
  {
    ...
    do {
      ...
      if(impl->owner != Network::my_node_id) {
        ...
        log_reservation.debug("forwarding reservation request: reservation=" IDFMT
                              ", from=%d, to=%d, mode=%d",
      ...
      if((impl->count == ReservationImpl::ZERO_COUNT) &&
        ...
        log_reservation.debug("granting reservation request: reservation=" IDFMT
                              ", node=%d, mode=%d",
                              args.lock.id, args.node, args.mode);
      ...
      //  work
      log_reservation.debug("deferring reservation request: reservation=" IDFMT
                            ", node=%d, mode=%d (count=%d cmode=%d)",
