...
namespace Realm {
						    ...
						    Event wait_on /*= Event::NO_EVENT*/)
  {
    ...
    for(size_t i = 0; i < n; i++) {
      ...
      if(l.empty()) {
        results[i] = r;
        Event added = SparsityMapRefCounter(results[i].sparsity.id).add_references(1);
        if(added.exists()) {
						   ...
						   Event wait_on /*= Event::NO_EVENT*/)
  {
    ...
    if(subspaces.empty()) {
      ...
      if(!was_inline || !result.dense()) {
        ...
        Event added = SparsityMapRefCounter(result.sparsity.id).add_references(1);
        if(added.exists()) {
          events.push_back(added);
        }
      }
  ...
  void IntersectionMicroOp<N,T>::populate_bitmask(BM& bitmask)
  {
	...
	while(it_lhs.valid && it_rhs.valid) {
	  // skip rectangles if they completely preceed the one on the other side
          ...
          // we have at least partial overlap - add the intersection and then consume
          // whichever
