...
namespace Realm {
  ...
  template <int N, typename T>
  /*static*/ SparsityMap<N, T>
  SparsityMap<N, T>::construct(const std::vector<Point<N, T>> &points, bool always_create,
                               bool disjoint)
  {
  ...
  template <int N, typename T>
  /*static*/ SparsityMap<N, T>
  SparsityMap<N, T>::construct(const std::vector<Rect<N, T>> &rects, bool always_create,
                               bool disjoint)
  {
    HybridRectangleList<N, T> hrl;
  ...
  class UnsubscribeAcknowledeged {
  public:
    UnsubscribeAcknowledeged(SparsityMapImplWrapper *_wrapper, NodeID _node)
      : wrapper(_wrapper)
      , node(_node)
    {}
    ...
    if(wait_on.has_triggered()) {
      ...
      if(remaining == count) {
        if(!subscribers.empty()) {
          // Increment the references by the number of expected responses
          // plus a guard to keep ourselves alive while we're still iterating
          // the subscribers and sending messages
  ...
  template <int N, typename T>
  SparsityMapImpl<N, T> *SparsityMapImplWrapper::get_or_create(SparsityMap<N, T> me)
  {
  ...
  /*static*/ ActiveMessageHandlerReg<typename SparsityMapImplWrapper::UnsubscribeMessage>
      SparsityMapImplWrapper::unsubscribe_message_handler_reg;
                     ...
                     std::vector<Rect<N, T>> &approx_rects)
  {
    ...
    if((bounds != 0) && (entries[first_span].lo[merge_dim] > bounds->lo[merge_dim]))
    ...
    if((bounds != 0) && (entries[last_span].hi[merge_dim] < bounds->hi[merge_dim]))
  ...
  template <int N, typename T>
  static bool any_overlaps(const Rect<N, T> &r, const std::vector<Rect<N, T>> &other,
  ...
  public:
    ...
    {
      for(int i = 0; i < N; i++) {
        if(a.lo[dim_order[i]] < b.lo[dim_order[i]])
          return true;
        if(a.lo[dim_order[i]] > b.lo[dim_order[i]])
          return false;
      }
                                                     ...
                                                     std::vector<Rect<N, T>> &covering)
  {
    ...
    if(N == 1) {
      ...
      bool ok = merge_1d_gaps(&bounds, 0 /*merge_dim*/, max_waste, max_rects,
                              SparsityMapToRectAdapter<N, T>(entries), merged);
      if(ok) {
      ...
      for(size_t i = 0; i < in_bounds.size(); i++) {
        ...
        if((i > 0) && (num_mismatched < 2))
          for(int j = 0; j < N; j++)
            if((mismatched_dim != j) &&
               ((to_cover[0].lo[j] != r.lo[j]) || (to_cover[0].hi[j] != r.hi[j]))) {
              mismatched_dim = j;
              num_mismatched++;
            }
      }
      ...
      if(num_mismatched == 1) {
        ...
        bool ok = merge_1d_gaps(&bounds, mismatched_dim, max_waste, max_rects, to_cover,
                                merged);
        if(ok) {
          covering.swap(merged);
          return true;
      ...
      log_cover.debug() << "vol=" << vol << " max_overhead=" << max_overhead
                        << " max_waste=" << max_waste;
      ...
      for(size_t i = 1; i < to_cover.size(); i++) {
        ...
        for(size_t k = 0; k < state.size(); k++)
          log_cover.debug() << "state[" << k << "]: waste=" << state[k].waste
                            << " clumps=" << PrettyVector<Rect<N, T>>(state[k].clumps);
        ...
        while(c-- > 0) {
          ...
          if(pc.waste < ~size_t(0)) {
            ...
            for(size_t j = 0; j <= c; j++) {
              if(!pc.clumps[j].overlaps(to_cover[i]))
                continue;
              touch_idx = j;
              contained = pc.clumps[j].contains(to_cover[i]);
              touch_count++;
              if(contained || (touch_count > 1))
                break; // early outs
            }
            ...
            if(touch_count == 1) {
              ...
              if(!any_overlaps(r, pc.clumps, touch_idx)) {
                ...
                merge_idx = touch_idx;
                merge_rect = r;
                best_waste = (pc.waste + (r.volume() - pc.clumps[touch_idx].volume()) -
                              to_cover[i].volume());
              }
            }
            ...
            if(touch_count == 0) {
              ...
              for(size_t j = 0; j <= c; j++) {
                ...
                if(!any_overlaps(r, pc.clumps, j)) {
                  // no conflicts - this is at least an option
                  size_t waste = (pc.waste + (r.volume() - pc.clumps[j].volume()) -
                                  to_cover[i].volume());
                  if(waste < best_waste) {
                    best_waste = waste;
                    merge_idx = j;
      ...
      for(size_t i = 1; i < state.size(); i++) {
        log_cover.debug() << "state[" << i << "]: waste=" << state[i].waste
                          << " clumps=" << PrettyVector<Rect<N, T>>(state[i].clumps);
      ...
      if(precise) {
        if(!this->entries_valid.load()) {
          ...
          if((NodeID(ID(me).sparsity_creator_node()) != Network::my_node_id) &&
            ...
            request_approx = !(this->approx_valid.load() || approx_requested);
  ...
  void SparsityMapImpl<N, T>::set_contributor_count(int count)
  {
    if(NodeID(ID(me).sparsity_creator_node()) == Network::my_node_id) {
      ...
      if(v == 0) {
        ...
        bool have_all_pieces =
            ((pcount == 0) ||
             ((remaining_piece_count.fetch_add_acqrel(pcount) + pcount) == 0));
        if(have_all_pieces)
          finalize();
      }
    } else {
  ...
  void SparsityMapImpl<N, T>::contribute_nothing(void)
  {
    ...
    if(owner != Network::my_node_id) {
      sparsity_comm->send_contribute(me, 1, 0, false);
      // send (the lack of) data to the owner to collect
      return;
    }

    // count is allowed to go negative if we get contributions before we know the total
    // expected
  ...
  template <int N, typename T>
  void
  SparsityMapImpl<N, T>::contribute_dense_rect_list(const std::vector<Rect<N, T>> &rects,
                                                    bool disjoint)
  {
    NodeID owner = ID(me).sparsity_creator_node();
  ...
  template <int N, typename T>
  void SparsityMapImpl<N, T>::remote_data_request(NodeID requestor, bool send_precise,
                                                  bool send_approx)
  {
    // first sanity check - we should be the owner of the data
    assert(NodeID(ID(me).sparsity_creator_node()) == Network::my_node_id);

    // take the long to determine atomically if we can send data or if we need to register
    // as a listener
    ...
    bool reply_approx = false;
    {
      ...
      remote_subscribers.add(requestor);
                                                ...
                                                bool reply_approx)
  {
    ...
    if(reply_precise) {
      log_part.info() << "sending precise data: sparsity=" << me
                      << " target=" << requestor;
  ...
  struct CompareBoundsLow {
    CompareBoundsLow(const int *_dim_order)
      : dim_order(_dim_order)
    {}
                                    ...
                                    std::vector<Rect<1, T>> &approx_rects, int max_rects)
  {
    ...
    // if not, then do a scan through the entries and remember the max_rects-1 largest
    // gaps - those
    ...
    //   std::cout << " " << gap_idxs[i] << "=" << gap_sizes[i] << ":" <<
    //   entries[gap_idxs[i]-1].bounds << "," << entries[gap_idxs[i]].bounds;
    // std::cout << " ]]]\n";
    // now just sort the gap indices so we can emit the right rectangles
    std::sort(gap_idxs.begin(), gap_idxs.end());
  ...
  /*static*/ ActiveMessageHandlerReg<
      typename SparsityMapImpl<N, T>::RemoteSparsityRequest>
      SparsityMapImpl<N, T>::remote_sparsity_request_reg;
  template <int N, typename T>
  /*static*/ ActiveMessageHandlerReg<
      typename SparsityMapImpl<N, T>::RemoteSparsityContrib>
      SparsityMapImpl<N, T>::remote_sparsity_contrib_reg;
  template <int N, typename T>
  /*static*/ ActiveMessageHandlerReg<
  ...
  template <int N, typename T>
  inline /*static*/ void SparsityMapImpl<N, T>::RemoteSparsityRequest::handle_message(
      NodeID sender, const SparsityMapImpl<N, T>::RemoteSparsityRequest &msg,
      const void *data, size_t datalen)
  {
    log_part.info() << "received sparsity request: sparsity=" << msg.sparsity
                    << " precise=" << msg.send_precise << " approx=" << msg.send_approx;
    SparsityMapImpl<N, T>::lookup(msg.sparsity)
        ->remote_data_request(sender, msg.send_precise, msg.send_approx);
  }
  ...
  template <int N, typename T>
  inline /*static*/ void SparsityMapImpl<N, T>::RemoteSparsityContrib::handle_message(
      NodeID sender, const SparsityMapImpl<N, T>::RemoteSparsityContrib &msg,
      const void *data, size_t datalen)
  {
    log_part.info() << "received remote contribution: sparsity=" << msg.sparsity
                    << " len=" << datalen;
  ...
  template <int N, typename T>
  inline /*static*/ void SparsityMapImpl<N, T>::SetContribCountMessage::handle_message(
      NodeID sender, const SparsityMapImpl<N, T>::SetContribCountMessage &msg,
      const void *data, size_t datalen)
  {
