...
namespace Realm {
  ...
  template <typename S>
  /*static*/ inline InstanceLayoutGeneric *
  InstanceLayoutGeneric::deserialize_new(S &deserializer)
  {
    return Serialization::PolymorphicSerdezHelper<InstanceLayoutGeneric>::deserialize_new(
        deserializer);
  }
  ...
  /*static*/ InstanceLayoutGeneric *InstanceLayoutGeneric::choose_instance_layout(
      IndexSpace<N, T> is, const InstanceLayoutConstraints &ilc, const int dim_order[N])
  {
  ...
  template <int N, typename T>
  /*static*/ InstanceLayoutGeneric *InstanceLayoutGeneric::choose_instance_layout(
      IndexSpace<N, T> is, const std::vector<Rect<N, T>> &covering,
      const InstanceLayoutConstraints &ilc, const int dim_order[N])
  {
    ...
    if(covering.empty()) {
      ...
      for(std::vector<InstanceLayoutConstraints::FieldGroup>::const_iterator it =
              ilc.field_groups.begin();
          it != ilc.field_groups.end(); ++it)
        for(std::vector<InstanceLayoutConstraints::FieldInfo>::const_iterator it2 =
                it->begin();
            it2 != it->end(); ++it2) {
  ...
  {
    return Serialization::PolymorphicSerdezHelper<InstanceLayoutGeneric>::serialize(
        serializer, ilg);
  }
  ...
  inline InstanceLayoutPieceBase::InstanceLayoutPieceBase(
      PieceLayoutTypes::LayoutType _layout_type)
  ...
  template <int N, typename T>
  inline InstanceLayoutPiece<N, T>::InstanceLayoutPiece(
      PieceLayoutTypes::LayoutType _layout_type)
  ...
  template <int N, typename T>
  inline InstanceLayoutPiece<N, T>::InstanceLayoutPiece(
      PieceLayoutTypes::LayoutType _layout_type, const Rect<N, T> &_bounds)
    : InstanceLayoutPieceBase(_layout_type)
    , bounds(_bounds)
  {}
  ...
  InstanceLayoutPiece<N, T>::deserialize_new(S &deserializer)
  {
    return Serialization::PolymorphicSerdezHelper<
        InstanceLayoutPiece<N, T>>::deserialize_new(deserializer);
  }

  template <int N, typename T, typename S>
  inline bool serialize(S &serializer, const InstanceLayoutPiece<N, T> &ilp)
  {
    return Serialization::PolymorphicSerdezHelper<InstanceLayoutPiece<N, T>>::serialize(
        serializer, ilp);
  }
  ...
  template <typename S>
  /*static*/ inline InstanceLayoutPiece<N, T> *
  AffineLayoutPiece<N, T>::deserialize_new(S &s)
  {
    AffineLayoutPiece<N, T> *alp = new AffineLayoutPiece<N, T>;
    if((s >> alp->bounds) && (s >> alp->strides) && (s >> alp->offset)) {
      return alp;
    } else {
      delete alp;
      return 0;
  ...
  void AffineLayoutPiece<N, T>::print(std::ostream &os) const
  {
    ...
    os << this->bounds << "->affine(" << strides << std::showpos
       << static_cast<int64_t>(offset) << std::noshowpos << ")";
  }
  ...
  AffineLayoutPiece<N, T>::create_lookup_inst(void *ptr, unsigned next_delta) const
  {
    PieceLookup::AffinePiece<N, T> *ap =
        new(ptr) PieceLookup::AffinePiece<N, T>(next_delta);
  ...
  {
    return ((s << this->bounds) && (s << strides) && (s << offset));
  }
  ...
  inline InstancePieceList<N, T>::~InstancePieceList(void)
  {
    for(typename std::vector<InstanceLayoutPiece<N, T> *>::const_iterator it =
            pieces.begin();
        it != pieces.end(); ++it)
      delete *it;
  }
  ...
  template <int N, typename T>
  inline const InstanceLayoutPiece<N, T> *
  InstancePieceList<N, T>::find_piece(Point<N, T> p) const
  {
    for(typename std::vector<InstanceLayoutPiece<N, T> *>::const_iterator it =
            pieces.begin();
        it != pieces.end(); ++it)
  ...
  namespace PieceLookup {
    ...
    template <int N, typename T>
    REALM_CUDA_HD unsigned AffinePiece<N, T>::delta() const
    {
      return (data >> 8);
    }

    template <int N, typename T>
    REALM_CUDA_HD const Instruction *AffinePiece<N, T>::next() const
    {
    ...
    template <int N, typename T>
    SplitPlane<N, T>::SplitPlane(int _split_dim, T _split_plane, unsigned _next_delta)
      : Instruction(Opcodes::OP_SPLIT1 + (_split_dim << 8) + (_next_delta << 16))
    ...
    {
      this->data = ((this->data & 0xffff) + (_next_delta << 16));
    }

    template <int N, typename T>
    REALM_CUDA_HD unsigned SplitPlane<N, T>::delta() const
    {
      return (data >> 16);
    }

    template <int N, typename T>
    REALM_CUDA_HD int SplitPlane<N, T>::split_dim() const
    {
      return (data >> 8) & 0xff;
    }

    template <int N, typename T>
    REALM_CUDA_HD const Instruction *SplitPlane<N, T>::next(const Point<N, T> &p) const
    {
    ...
    template <int N, typename T>
    REALM_CUDA_HD bool SplitPlane<N, T>::splits_rect(const Rect<N, T> &r) const
    {
  ...
  template <typename FT, int N, typename T>
  inline /*static*/ bool
  GenericAccessor<FT, N, T>::is_compatible(RegionInstance inst, size_t field_offset,
                                           const Rect<N, T> &subrect)
  {
    return true;
  }
  ...
  template <typename INST>
  inline /*static*/ bool
  GenericAccessor<FT, N, T>::is_compatible(const INST &instance, unsigned field_id,
                                           const Rect<N, T> &subrect)
  {
    return true;
  }
  ...
  template <typename FT, int N, typename T>
  REALM_CUDA_HD inline void AffineAccessor<FT, N, T>::reset()
  {
    base = 0;
  }
  ...
  template <typename FT, int N, typename T>
  inline void AffineAccessor<FT, N, T>::reset(RegionInstance inst, FieldID field_id,
                                              size_t subfield_offset /*= 0*/)
  {
    const InstanceLayout<N, T> *layout =
        checked_cast<const InstanceLayout<N, T> *>(inst.get_layout());
  ...
  template <typename FT, int N, typename T>
  AffineAccessor<FT, N, T>::AffineAccessor(RegionInstance inst, FieldID field_id,
                                           const Rect<N, T> &subrect,
                                           size_t subfield_offset /*= 0*/)
  {
  ...
  template <typename FT, int N, typename T>
  void AffineAccessor<FT, N, T>::reset(RegionInstance inst, FieldID field_id,
                                       const Rect<N, T> &subrect,
                                       size_t subfield_offset /*= 0*/)
  {
    const InstanceLayout<N, T> *layout =
  ...
  template <int N2, typename T2>
  inline void
  AffineAccessor<FT, N, T>::reset(RegionInstance inst, const Matrix<N2, N, T2> &transform,
  ...
  template <typename FT, int N, typename T>
  inline /*static*/ bool
  AffineAccessor<FT, N, T>::is_compatible(RegionInstance inst, FieldID field_id,
                                          const Rect<N, T> &subrect)
  {
    const InstanceLayout<N, T> *layout =
        checked_cast<const InstanceLayout<N, T> *>(inst.get_layout());
  ...
  template <int N2, typename T2>
  inline /*static*/ bool
  AffineAccessor<FT, N, T>::is_compatible(RegionInstance inst,
                                          const Matrix<N2, N, T2> &transform,
                                          const Point<N2, T2> &offset, FieldID field_id)
  {
    // instance's dimensionality should be <N2,T2>
  ...
  template <typename FT, int N, typename T>
  template <int N2, typename T2>
  inline /*static*/ bool AffineAccessor<FT, N, T>::is_compatible(
  ...
  template <typename FT, int N, typename T>
  REALM_CUDA_HD inline void AffineAccessor<FT, N, T>::write(const Point<N, T> &p,
                                                            FT newval) const
  {
  ...
  template <typename FT, int N, typename T>
  REALM_CUDA_HD inline FT &
  AffineAccessor<FT, N, T>::operator[](const Point<N, T> &p) const
  {
    return *(this->get_ptr(p));
  }

  template <typename FT, int N, typename T>
  REALM_CUDA_HD inline bool
  AffineAccessor<FT, N, T>::is_dense_arbitrary(const Rect<N, T> &bounds) const
  {
  ...
  template <typename FT, int N, typename T>
  REALM_CUDA_HD inline bool
  AffineAccessor<FT, N, T>::is_dense_col_major(const Rect<N, T> &bounds) const
  {
    size_t exp_offset = sizeof(FT);
    for(int i = 0; i < N; i++) {
  ...
  template <typename FT, int N, typename T>
  REALM_CUDA_HD inline bool
  AffineAccessor<FT, N, T>::is_dense_row_major(const Rect<N, T> &bounds) const
  {
    size_t exp_offset = sizeof(FT);
    for(int i = N - 1; i >= 0; i--) {
      if(strides[i] != exp_offset) {
  ...
  template <typename FT, int N, typename T>
  REALM_CUDA_HD inline FT *AffineAccessor<FT, N, T>::get_ptr(const Point<N, T> &p) const
  {
...
#include "realm/realm_assert.h"
  ...
  {
    typename DeferType<Kokkos::LayoutStride, FT>::type kls;
    // deduce the begins array type from the OffsetView itself (a dependent
    //  name, resolved only at instantiation) so that this header need not
    //  name Kokkos::Array, whose template signature differs between Kokkos
    //  4 and 5
    typename Kokkos::Experimental::OffsetView<Args...>::begins_type begins;
    uintptr_t base_shifted = base;
    for(int i = 0; i < N; i++) {
  ...
  {
    os << "AffineAccessor{ base=" << std::hex << a.base << std::dec
       << " strides=" << a.strides;
#ifdef REALM_ACCESSOR_DEBUG
    os << " inst=" << a.dbg_inst;
#endif
#if defined(REALM_ACCESSOR_DEBUG) || defined(REALM_USE_KOKKOS)
    os << " bounds=" << a.bounds;
    os << "->[" << std::hex << a.ptr(a.bounds.lo) << "," << a.ptr(a.bounds.hi) + 1
       << std::dec << "]";
#endif
    os << " }";
    return os;
  ...
  template <typename FT, int N, typename T>
  REALM_CUDA_HD MultiAffineAccessor<FT, N, T>::MultiAffineAccessor(void)
  {
    reset();
  }
  ...
  template <typename FT, int N, typename T>
  REALM_CUDA_HD void MultiAffineAccessor<FT, N, T>::reset()
  {
    start_inst = 0;
  ...
  template <typename FT, int N, typename T>
  void MultiAffineAccessor<FT, N, T>::reset(RegionInstance inst, FieldID field_id,
                                            size_t subfield_offset /*= 0*/)
  {
    unsigned allowed_mask = (PieceLookup::ALLOW_AFFINE_PIECE | PieceLookup::ALLOW_SPLIT1);
  ...
  template <typename FT, int N, typename T>
  REALM_CUDA_HD FT *MultiAffineAccessor<FT, N, T>::ptr(const Point<N, T> &p) const
  {
  ...
  template <typename FT, int N, typename T>
  REALM_CUDA_HD FT *MultiAffineAccessor<FT, N, T>::ptr(const Rect<N, T> &r,
                                                       size_t strides[N]) const
  {
    ...
    if((start_inst == 0) || (piece_valid && piece_bounds.contains(r))) {
      uintptr_t rawptr = piece_base;
      for(int i = 0; i < N; i++)
        rawptr += r.lo[i] * piece_strides[i];
      for(int i = 0; i < N; i++)
        strides[i] = piece_strides[i];
      return reinterpret_cast<FT *>(rawptr);
    } else {
      const PieceLookup::Instruction *i = start_inst;
      while(true) {
  ...
  template <typename FT, int N, typename T>
  REALM_CUDA_HD FT MultiAffineAccessor<FT, N, T>::read(const Point<N, T> &p) const
  {
    return *(this->ptr(p));
  }

  template <typename FT, int N, typename T>
  REALM_CUDA_HD void MultiAffineAccessor<FT, N, T>::write(const Point<N, T> &p,
                                                          FT newval) const
  {
    *(this->ptr(p)) = newval;
  }
  ...
  template <typename FT, int N, typename T>
  REALM_CUDA_HD FT &MultiAffineAccessor<FT, N, T>::operator[](const Point<N, T> &p) const
  {
    return *(this->ptr(p));
  }

  template <typename FT, int N, typename T>
  REALM_CUDA_HD FT *MultiAffineAccessor<FT, N, T>::ptr(const Point<N, T> &p)
  {
  ...
  template <typename FT, int N, typename T>
  REALM_CUDA_HD FT MultiAffineAccessor<FT, N, T>::read(const Point<N, T> &p)
  {
    return *(this->ptr(p));
  }

  template <typename FT, int N, typename T>
  REALM_CUDA_HD void MultiAffineAccessor<FT, N, T>::write(const Point<N, T> &p, FT newval)
  {
    *(this->ptr(p)) = newval;
  }

  template <typename FT, int N, typename T>
  REALM_CUDA_HD FT &MultiAffineAccessor<FT, N, T>::operator[](const Point<N, T> &p)
  {
    return *(this->ptr(p));
  }
  ...
  std::ostream &operator<<(std::ostream &os, const MultiAffineAccessor<FT, N, T> &a)
  {
    ...
    if(a.piece_valid)
      os << " cache={ bounds=" << a.piece_bounds << " base=" << std::hex << a.piece_base
         << std::dec << " strides=" << a.piece_strides << " }";
    os << " }";
    return os;
  }
