...
struct OutgoingMessage {
  ...
  void set_payload(PayloadSource *_payload, size_t _payload_size, int _payload_mode,
                   void *_dstptr = 0);
...
public:
  ...
  static void release_srcptr_handler(gasnet_token_t token, gasnet_handlerarg_t arg0,
...
SrcDataPool::~SrcDataPool(void)
{
  size_t total = 0;
  size_t nonzero = 0;
  for(std::map<void *, ssize_t>::const_iterator it = alloc_counts.begin();
      it != alloc_counts.end(); it++) {
    total++;
    if(it->second != 0) {
      printf("HELP!  srcptr %p on node %d has final count of %zd\n", it->first,
             gasnet_mynode(), it->second);
      nonzero++;
    }
  }
...
void SrcDataPool::release_srcptr(void *srcptr)
{
  ...
  std::vector<std::pair<OutgoingMessage *, void *>> satisfied;
  {
    ...
    if(!free_list.empty()) {
      ...
      while(above != free_list.begin()) {
        std::map<char *, size_t>::iterator below = above;
        below--;

        log_sdp.spew("merge?  %p+%zd %p+%zd NONE", below->first, below->second, srcptr_c,
                     size);
      ...
      while(above != free_list.end()) {
        log_sdp.spew("merge?  NONE %p+%zd %p+%zd", srcptr_c, size, above->first,
                     above->second);
  ...
  //   do their copies
  if(!satisfied.empty())
    for(std::vector<std::pair<OutgoingMessage *, void *>>::iterator it =
            satisfied.begin();
        it != satisfied.end(); it++) {
      log_sdp.debug("satisfying pending allocation: %p for %p", it->second, it->first);
                                     ...
                                     bool first_try)
{
  ...
  while(true) {
    ...
    if((max_spill_bytes != 0) && !ThreadLocal::always_allow_spilling &&
       !ThreadLocal::in_message_handler && (new_spill_bytes > max_spill_bytes))
      break;

    if(current_spill_bytes.compare_exchange(old_spill_bytes, new_spill_bytes)) {
...
#ifdef TRACK_PER_MESSAGE_SPILLING
      size_t new_permsg_spill_bytes =
          current_permsg_spill_bytes[msgtype].fetch_add(size_needed) + size_needed;
  ...
  log_spill.debug() << "max spill amount reached - suspension required ("
                    << current_spill_bytes.load() << " + " << size_needed << " > "
                    << max_spill_bytes;

  // if this is the first try for this message, increase the total waiter count and
  // complain
  ...
  while((current_spill_bytes.load() + size_needed) > max_spill_bytes) {
    condvar.wait();
    log_spill.debug() << "awake - rechecking: " << current_spill_bytes.load() << " + "
                      << size_needed << " > " << max_spill_bytes << "?";
  }
...
OutgoingMessage::~OutgoingMessage(void)
{
  if((payload_mode == PAYLOAD_COPY) || (payload_mode == PAYLOAD_FREE)) {
    if(payload_size > 0) {
      ...
      printf("%d: freeing payload %x = [%p, %p)\n", gasnet_mynode(), payload_num, payload,
...
#ifdef DETAILED_MESSAGE_TIMING
static const size_t DEFAULT_MESSAGE_MAX_COUNT =
    16 << 20; // 16 million messages should be plenty
...
public:
  ...
  ~DetailedMessageTiming(void) { delete[] message_timing; }
  ...
  void record(int index, int peer, unsigned short msg_id, char write_lmb,
              unsigned msg_size, unsigned queue_depth, const CurrentTime &t_start,
              const CurrentTime &t_end)
  {
    if((index >= 0) && (index < message_max_count)) {
      ...
      unsigned long long delta =
          (t_end.sec - t_start.sec) * 1000000000ULL + t_end.nsec - t_start.nsec;
#endif
...
public:
  ActiveMessageEndpoint(gasnet_node_t _peer)
    : peer(_peer)
    , cond(mutex)
    , out_channel_closed(false)
    , messages_sent(0)
    , in_channel_closed(false)
    , messages_handled(0)
    , messages_expected(size_t(-1))
  {
    cur_write_lmb = 0;
    cur_write_offset = 0;
    cur_write_count = 0;
    ...
    for(int i = 0; i < num_lmbs; i++) {
      lmb_w_bases[i] =
          ((char *)(segment_info[peer].addr)) +
          (segment_info[peer].size - lmb_size * (gasnet_mynode() * num_lmbs + i + 1));
      lmb_r_bases[i] = ((char *)(segment_info[gasnet_mynode()].addr)) +
                       (segment_info[peer].size - lmb_size * (peer * num_lmbs + i + 1));
      lmb_r_counts[i] = 0;
      lmb_w_avail[i] = true;
    }
  ...
  bool push_messages(int max_to_send, bool wait, TimeLimit work_until)
  {
    ...
    while(still_more && ((max_to_send == 0) || (count < max_to_send))) {
      ...
      // short messages are used primarily for flow control, so always try to send those
      // first (if a short message needs to be ordered with long messages, it goes in the
      // long message queue)
      if(out_short_hdrs.size() > 0) {
        ...
        unsigned qdepth =
            out_long_hdrs.size(); // sic - we assume the short queue is always near-empty
        message_log_state = MSGLOGSTATE_NORMAL;
#endif
        ...
        send_short(hdr);
#ifdef DETAILED_MESSAGE_TIMING
        detailed_message_timing.record(timing_idx, peer, hdr->msgid, -1,
                                       hdr->num_args * 4, qdepth, start_time,
                                       CurrentTime());
#endif
        delete hdr;
        count++;
      ...
      if(out_long_hdrs.size() > 0) {
        ...
        // no payload?  this happens when a short/medium message needs to be ordered with
        // long messages
...
#ifdef DETAILED_MESSAGE_TIMING
          CurrentTime now;
          detailed_message_timing.record(timing_idx, peer, hdr->msgid, -2,
                                         hdr->num_args * 4 + hdr->payload_size, qdepth,
                                         now, now);
#endif
          break;
        }

        // do we have a known destination pointer on the target?  if so, no need to use
        // LMB
        if(hdr->dstptr != 0) {
...
#ifdef DETAILED_MESSAGE_TIMING
          CurrentTime now;
          detailed_message_timing.record(timing_idx, peer, hdr->msgid, -3,
                                         hdr->num_args * 4 + hdr->payload_size, qdepth,
                                         now, now);
#endif
          break;
        ...
        if((cur_write_offset + hdr->payload_size) <= lmb_size) {
          ...
          printf("LMB: sending %zd bytes %d->%d, [%p,%p)\n", hdr->payload_size,
                 gasnet_mynode(), peer, dest_ptr, dest_ptr + hdr->payload_size);
#endif
#ifdef DETAILED_MESSAGE_TIMING
          CurrentTime start_time;
#endif
...
#ifdef DETAILED_MESSAGE_TIMING
          detailed_message_timing.record(timing_idx, peer, hdr->msgid, cur_write_lmb,
                                         hdr->num_args * 4 + hdr->payload_size, qdepth,
                                         start_time, CurrentTime());
#endif
          delete hdr;
          ...
          printf("LMB: flipping buffer %d for %d->%d, [%p,%p), count=%d\n", flip_buffer,
                 gasnet_mynode(), peer, lmb_w_bases[flip_buffer],
                 lmb_w_bases[flip_buffer] + lmb_size, flip_count);
#endif
#ifdef ACTIVE_MESSAGE_TRACE
          log_amsg_trace.info("Active Message Request: %d %d 2 0", MSGID_FLIP_REQ, peer);
#endif
#ifdef DETAILED_MESSAGE_TIMING
          CurrentTime start_time;
#endif
          CHECK_GASNET(
              gasnet_AMRequestShort2(peer, MSGID_FLIP_REQ, flip_buffer, flip_count));
#ifdef DETAILED_MESSAGE_TIMING
          detailed_message_timing.record(timing_idx, peer, MSGID_FLIP_REQ, flip_buffer, 8,
                                         qdepth, start_time, CurrentTime());
#endif
#ifdef TRACE_MESSAGES
          sent_messages.fetch_add(1);
#endif
  ...
  bool enqueue_message(OutgoingMessage *hdr, bool in_order)
  {
    ...
    //  allocate spill data
    if((hdr->payload_size > 0) &&
       ((hdr->payload_mode == PAYLOAD_COPY) || (hdr->payload_mode == PAYLOAD_FREE))) {
      SrcDataPool::Lock held_lock(*srcdatapool);
      bool first_try = true;
      while(!srcdatapool->alloc_spill_memory(hdr->payload_size, hdr->msgid, held_lock,
                                             first_try)) {
    ...
    if(out_channel_closed) {
      if(hdr->msgid == MSGID_NEW_ACTIVEMSG) {
        ...
        if(strict_shutdown) {
          log_amsg.fatal() << "post-shutdown message: dst=" << peer
                           << " type=" << activemsg_handler_table.lookup_message_name(id);
          abort();
        } else
          log_amsg.warning() << "dropping post-shutdown message - hang possible: dst="
                             << peer << " type="
                             << activemsg_handler_table.lookup_message_name(id);
      } else {
  ...
  bool handle_long_msgptr(int msgptr_index)
  {
    ...
    printf("LMB: received %p for %d->%d in buffer %d, [%p, %p)\n", ptr, peer,
           gasnet_mynode(), r_buffer, lmb_r_bases[r_buffer],
    ...
    if(lmb_r_counts[r_buffer] == 0) {
      ...
      printf("LMB: acking flip of buffer %d for %d->%d, [%p,%p)\n", r_buffer, peer,
             gasnet_mynode(), lmb_r_bases[r_buffer], lmb_r_bases[r_buffer] + lmb_size);
#endif
  ...
  bool adjust_long_msgsize(void *&ptr, size_t &buffer_size, int frag_info)
  {
...
#ifdef DEBUG_AMREQUESTS
    printf("%d: adjust(%p, %zd, %d, %d)\n", gasnet_mynode(), ptr, buffer_size, message_id,
           chunks);
#endif
    ...
    std::map<int, ChunkInfo>::iterator finder = observed_messages.find(message_id);
  ...
  bool handle_flip_request(int buffer, int count)
  {
    ...
    printf("LMB: received flip of buffer %d for %d->%d, [%p,%p), count=%d\n", buffer,
           peer, gasnet_mynode(), lmb_r_bases[buffer], lmb_r_bases[buffer] + lmb_size,
           count);
#endif
#ifdef TRACE_MESSAGES
...
#ifdef DEBUG_LMB
      printf("LMB: acking flip of buffer %d for %d->%d, [%p,%p)\n", buffer, peer,
             gasnet_mynode(), lmb_r_bases[buffer], lmb_r_bases[buffer] + lmb_size);
#endif
  ...
  void handle_flip_ack(int buffer)
  {
    ...
    printf("LMB: received flip ack of buffer %d for %d->%d, [%p,%p)\n", buffer,
           gasnet_mynode(), peer, lmb_w_bases[buffer], lmb_w_bases[buffer] + lmb_size);
#endif
#ifdef TRACE_MESSAGES
    received_messages.fetch_add(1);
#endif
  ...
  void flush_outbound_messages(bool is_quiescent)
  {
    ...
    log_amsg.info() << "sending flush: " << gasnet_mynode() << "->" << peer
                    << ": is_quiescent=" << is_quiescent << " count=" << sent_snapshot;
    handlerarg_t count_hi = sent_snapshot >> 32;
    handlerarg_t count_lo = sent_snapshot & 0xFFFFFFFFULL;
    CHECK_GASNET(gasnet_AMRequestShort3(peer, MSGID_CHANNEL_FLUSH, count_lo, count_hi,
                                        is_quiescent));
  }
                            ...
                            handlerarg_t is_quiescent)
  {
    ...
    log_amsg.info() << "received flush: " << peer << "->" << gasnet_mynode()
                    << ": count=" << count << " cur=" << messages_handled.load()
...
protected:
  void send_short(OutgoingMessage *hdr)
  {
...
#ifdef DEBUG_AMREQUESTS
    printf("%d->%d: %s %d %d %p %zd / %x %x %x %x / %x %x %x %x / %x %x %x %x / %x %x %x "
           "%x\n",
           gasnet_mynode(), peer,
           ((hdr->payload_mode == PAYLOAD_NONE) ? "SHORT" : "MEDIUM"), hdr->num_args,
           hdr->msgid, hdr->payload, hdr->payload_size, hdr->args[0], hdr->args[1],
           hdr->args[2], hdr->args[3], hdr->args[4], hdr->args[5], hdr->args[6],
           hdr->args[7], hdr->args[8], hdr->args[9], hdr->args[10], hdr->args[11],
...
#endif
#ifdef ACTIVE_MESSAGE_TRACE
    log_amsg_trace.info("Active Message Request: %d %d %d %ld", hdr->msgid, peer,
                        hdr->num_args,
                        (hdr->payload_mode == PAYLOAD_NONE) ? 0 : hdr->payload_size);
#endif
    switch(hdr->num_args) {
    case 1:
    ...
    case 2:
      if(hdr->payload_mode != PAYLOAD_NONE) {
        CHECK_GASNET(gasnet_AMRequestMedium2(peer, hdr->msgid, hdr->payload,
                                             hdr->payload_size, hdr->args[0],
                                             hdr->args[1]));
      } else {
        CHECK_GASNET(
    ...
    case 3:
      if(hdr->payload_mode != PAYLOAD_NONE) {
        CHECK_GASNET(gasnet_AMRequestMedium3(peer, hdr->msgid, hdr->payload,
                                             hdr->payload_size, hdr->args[0],
                                             hdr->args[1], hdr->args[2]));
      } else {
        CHECK_GASNET(gasnet_AMRequestShort3(peer, hdr->msgid, hdr->args[0], hdr->args[1],
                                            hdr->args[2]));
      }
      break;
    ...
    case 4:
      if(hdr->payload_mode != PAYLOAD_NONE) {
        CHECK_GASNET(gasnet_AMRequestMedium4(peer, hdr->msgid, hdr->payload,
                                             hdr->payload_size, hdr->args[0],
                                             hdr->args[1], hdr->args[2], hdr->args[3]));
      } else {
        CHECK_GASNET(gasnet_AMRequestShort4(peer, hdr->msgid, hdr->args[0], hdr->args[1],
                                            hdr->args[2], hdr->args[3]));
      }
      break;

    case 5:
      if(hdr->payload_mode != PAYLOAD_NONE) {
        CHECK_GASNET(gasnet_AMRequestMedium5(
            peer, hdr->msgid, hdr->payload, hdr->payload_size, hdr->args[0], hdr->args[1],
            hdr->args[2], hdr->args[3], hdr->args[4]));
      } else {
        CHECK_GASNET(gasnet_AMRequestShort5(peer, hdr->msgid, hdr->args[0], hdr->args[1],
                                            hdr->args[2], hdr->args[3], hdr->args[4]));
      }
      break;
    ...
    case 6:
      if(hdr->payload_mode != PAYLOAD_NONE) {
        CHECK_GASNET(gasnet_AMRequestMedium6(
            peer, hdr->msgid, hdr->payload, hdr->payload_size, hdr->args[0], hdr->args[1],
            hdr->args[2], hdr->args[3], hdr->args[4], hdr->args[5]));
      } else {
        CHECK_GASNET(gasnet_AMRequestShort6(peer, hdr->msgid, hdr->args[0], hdr->args[1],
                                            hdr->args[2], hdr->args[3], hdr->args[4],
                                            hdr->args[5]));
      }
      break;
    ...
    case 7:
      if(hdr->payload_mode != PAYLOAD_NONE) {
        CHECK_GASNET(gasnet_AMRequestMedium7(
            peer, hdr->msgid, hdr->payload, hdr->payload_size, hdr->args[0], hdr->args[1],
            hdr->args[2], hdr->args[3], hdr->args[4], hdr->args[5], hdr->args[6]));
      } else {
        CHECK_GASNET(gasnet_AMRequestShort7(peer, hdr->msgid, hdr->args[0], hdr->args[1],
                                            hdr->args[2], hdr->args[3], hdr->args[4],
                                            hdr->args[5], hdr->args[6]));
      }
      break;

    case 8:
      if(hdr->payload_mode != PAYLOAD_NONE) {
        CHECK_GASNET(gasnet_AMRequestMedium8(
            peer, hdr->msgid, hdr->payload, hdr->payload_size, hdr->args[0], hdr->args[1],
            hdr->args[2], hdr->args[3], hdr->args[4], hdr->args[5], hdr->args[6],
            hdr->args[7]));
      } else {
        CHECK_GASNET(gasnet_AMRequestShort8(peer, hdr->msgid, hdr->args[0], hdr->args[1],
                                            hdr->args[2], hdr->args[3], hdr->args[4],
                                            hdr->args[5], hdr->args[6], hdr->args[7]));
      }
      break;

    case 9:
      if(hdr->payload_mode != PAYLOAD_NONE) {
        CHECK_GASNET(gasnet_AMRequestMedium9(
            peer, hdr->msgid, hdr->payload, hdr->payload_size, hdr->args[0], hdr->args[1],
            hdr->args[2], hdr->args[3], hdr->args[4], hdr->args[5], hdr->args[6],
            hdr->args[7], hdr->args[8]));
      } else {
        CHECK_GASNET(gasnet_AMRequestShort9(
            peer, hdr->msgid, hdr->args[0], hdr->args[1], hdr->args[2], hdr->args[3],
            hdr->args[4], hdr->args[5], hdr->args[6], hdr->args[7], hdr->args[8]));
      }
      break;

    case 10:
      if(hdr->payload_mode != PAYLOAD_NONE) {
        CHECK_GASNET(gasnet_AMRequestMedium10(
            peer, hdr->msgid, hdr->payload, hdr->payload_size, hdr->args[0], hdr->args[1],
            hdr->args[2], hdr->args[3], hdr->args[4], hdr->args[5], hdr->args[6],
            hdr->args[7], hdr->args[8], hdr->args[9]));
      } else {
        CHECK_GASNET(gasnet_AMRequestShort10(peer, hdr->msgid, hdr->args[0], hdr->args[1],
                                             hdr->args[2], hdr->args[3], hdr->args[4],
                                             hdr->args[5], hdr->args[6], hdr->args[7],
                                             hdr->args[8], hdr->args[9]));
      }
      break;

    case 11:
      if(hdr->payload_mode != PAYLOAD_NONE) {
        CHECK_GASNET(gasnet_AMRequestMedium11(
            peer, hdr->msgid, hdr->payload, hdr->payload_size, hdr->args[0], hdr->args[1],
            hdr->args[2], hdr->args[3], hdr->args[4], hdr->args[5], hdr->args[6],
            hdr->args[7], hdr->args[8], hdr->args[9], hdr->args[10]));
      } else {
        CHECK_GASNET(gasnet_AMRequestShort11(peer, hdr->msgid, hdr->args[0], hdr->args[1],
                                             hdr->args[2], hdr->args[3], hdr->args[4],
                                             hdr->args[5], hdr->args[6], hdr->args[7],
                                             hdr->args[8], hdr->args[9], hdr->args[10]));
      }
      break;

    case 12:
      if(hdr->payload_mode != PAYLOAD_NONE) {
        CHECK_GASNET(gasnet_AMRequestMedium12(
            peer, hdr->msgid, hdr->payload, hdr->payload_size, hdr->args[0], hdr->args[1],
            hdr->args[2], hdr->args[3], hdr->args[4], hdr->args[5], hdr->args[6],
            hdr->args[7], hdr->args[8], hdr->args[9], hdr->args[10], hdr->args[11]));
      } else {
        CHECK_GASNET(gasnet_AMRequestShort12(
            peer, hdr->msgid, hdr->args[0], hdr->args[1], hdr->args[2], hdr->args[3],
            hdr->args[4], hdr->args[5], hdr->args[6], hdr->args[7], hdr->args[8],
    ...
    case 13:
      if(hdr->payload_mode != PAYLOAD_NONE) {
        CHECK_GASNET(gasnet_AMRequestMedium13(
            peer, hdr->msgid, hdr->payload, hdr->payload_size, hdr->args[0], hdr->args[1],
            hdr->args[2], hdr->args[3], hdr->args[4], hdr->args[5], hdr->args[6],
            hdr->args[7], hdr->args[8], hdr->args[9], hdr->args[10], hdr->args[11],
            hdr->args[12]));
      } else {
        CHECK_GASNET(gasnet_AMRequestShort13(
            peer, hdr->msgid, hdr->args[0], hdr->args[1], hdr->args[2], hdr->args[3],
            hdr->args[4], hdr->args[5], hdr->args[6], hdr->args[7], hdr->args[8],
            hdr->args[9], hdr->args[10], hdr->args[11], hdr->args[12]));
      }
      break;

    case 14:
      if(hdr->payload_mode != PAYLOAD_NONE) {
        CHECK_GASNET(gasnet_AMRequestMedium14(
            peer, hdr->msgid, hdr->payload, hdr->payload_size, hdr->args[0], hdr->args[1],
            hdr->args[2], hdr->args[3], hdr->args[4], hdr->args[5], hdr->args[6],
            hdr->args[7], hdr->args[8], hdr->args[9], hdr->args[10], hdr->args[11],
            hdr->args[12], hdr->args[13]));
      } else {
        CHECK_GASNET(gasnet_AMRequestShort14(
            peer, hdr->msgid, hdr->args[0], hdr->args[1], hdr->args[2], hdr->args[3],
            hdr->args[4], hdr->args[5], hdr->args[6], hdr->args[7], hdr->args[8],
            hdr->args[9], hdr->args[10], hdr->args[11], hdr->args[12], hdr->args[13]));
      }
      break;

    case 15:
      if(hdr->payload_mode != PAYLOAD_NONE) {
        CHECK_GASNET(gasnet_AMRequestMedium15(
            peer, hdr->msgid, hdr->payload, hdr->payload_size, hdr->args[0], hdr->args[1],
            hdr->args[2], hdr->args[3], hdr->args[4], hdr->args[5], hdr->args[6],
            hdr->args[7], hdr->args[8], hdr->args[9], hdr->args[10], hdr->args[11],
            hdr->args[12], hdr->args[13], hdr->args[14]));
      } else {
        CHECK_GASNET(gasnet_AMRequestShort15(
            peer, hdr->msgid, hdr->args[0], hdr->args[1], hdr->args[2], hdr->args[3],
            hdr->args[4], hdr->args[5], hdr->args[6], hdr->args[7], hdr->args[8],
            hdr->args[9], hdr->args[10], hdr->args[11], hdr->args[12], hdr->args[13],
            hdr->args[14]));
    ...
    case 16:
      if(hdr->payload_mode != PAYLOAD_NONE) {
        CHECK_GASNET(gasnet_AMRequestMedium16(
            peer, hdr->msgid, hdr->payload, hdr->payload_size, hdr->args[0], hdr->args[1],
            hdr->args[2], hdr->args[3], hdr->args[4], hdr->args[5], hdr->args[6],
            hdr->args[7], hdr->args[8], hdr->args[9], hdr->args[10], hdr->args[11],
            hdr->args[12], hdr->args[13], hdr->args[14], hdr->args[15]));
      } else {
        CHECK_GASNET(gasnet_AMRequestShort16(
            peer, hdr->msgid, hdr->args[0], hdr->args[1], hdr->args[2], hdr->args[3],
            hdr->args[4], hdr->args[5], hdr->args[6], hdr->args[7], hdr->args[8],
            hdr->args[9], hdr->args[10], hdr->args[11], hdr->args[12], hdr->args[13],
            hdr->args[14], hdr->args[15]));
      }
      break;
  ...
  void send_long(OutgoingMessage *hdr, void *dest_ptr)
  {
    ...
    if(hdr->payload_size <= max_long_req) {
      ...
      hdr->args[info_start + 0] = ((next_outgoing_message_id << 12) + chunks);
    ...
    for(int i = (chunks - 1); i >= 0; i--) {
      ...
      size_t size =
          ((i < (chunks - 1)) ? max_long_req
                              : (hdr->payload_size - (chunks - 1) * max_long_req));
#ifdef DEBUG_AMREQUESTS
      printf("%d->%d: LONG %d %d %p %zd %p / %x %x %x %x / %x %x %x %x / %x %x %x %x / "
             "%x %x %x %x\n",
             gasnet_mynode(), peer, hdr->num_args, hdr->msgid,
             ((char *)hdr->payload) + (i * max_long_req), size,
             ((char *)dest_ptr) + (i * max_long_req), hdr->args[0], hdr->args[1],
             hdr->args[2], hdr->args[3], hdr->args[4], hdr->args[5], hdr->args[6],
             hdr->args[7], hdr->args[8], hdr->args[9], hdr->args[10], hdr->args[11],
             hdr->args[12], hdr->args[13], hdr->args[14], hdr->args[15]);
      fflush(stdout);
      ...
      case 2:
        CHECK_GASNET(gasnet_AMRequestLongAsync2(
            peer, hdr->msgid, ((char *)hdr->payload) + (i * max_long_req), size,
            ((char *)dest_ptr) + (i * max_long_req), hdr->args[0], hdr->args[1]));
        break;

      case 3:
        CHECK_GASNET(gasnet_AMRequestLongAsync3(
            peer, hdr->msgid, ((char *)hdr->payload) + (i * max_long_req), size,
            ((char *)dest_ptr) + (i * max_long_req), hdr->args[0], hdr->args[1],
            hdr->args[2]));
        break;
      ...
      case 4:
        CHECK_GASNET(gasnet_AMRequestLongAsync4(
            peer, hdr->msgid, ((char *)hdr->payload) + (i * max_long_req), size,
            ((char *)dest_ptr) + (i * max_long_req), hdr->args[0], hdr->args[1],
            hdr->args[2], hdr->args[3]));
        break;
      case 5:
        CHECK_GASNET(gasnet_AMRequestLongAsync5(
            peer, hdr->msgid, ((char *)hdr->payload) + (i * max_long_req), size,
            ((char *)dest_ptr) + (i * max_long_req), hdr->args[0], hdr->args[1],
            hdr->args[2], hdr->args[3], hdr->args[4]));
        break;
      case 6:
        CHECK_GASNET(gasnet_AMRequestLongAsync6(
            peer, hdr->msgid, ((char *)hdr->payload) + (i * max_long_req), size,
            ((char *)dest_ptr) + (i * max_long_req), hdr->args[0], hdr->args[1],
            hdr->args[2], hdr->args[3], hdr->args[4], hdr->args[5]));
        break;
      case 7:
        CHECK_GASNET(gasnet_AMRequestLongAsync7(
            peer, hdr->msgid, ((char *)hdr->payload) + (i * max_long_req), size,
            ((char *)dest_ptr) + (i * max_long_req), hdr->args[0], hdr->args[1],
            hdr->args[2], hdr->args[3], hdr->args[4], hdr->args[5], hdr->args[6]));
        break;
      case 8:
        CHECK_GASNET(gasnet_AMRequestLongAsync8(
            peer, hdr->msgid, ((char *)hdr->payload) + (i * max_long_req), size,
            ((char *)dest_ptr) + (i * max_long_req), hdr->args[0], hdr->args[1],
            hdr->args[2], hdr->args[3], hdr->args[4], hdr->args[5], hdr->args[6],
            hdr->args[7]));
        break;
      case 9:
        CHECK_GASNET(gasnet_AMRequestLongAsync9(
            peer, hdr->msgid, ((char *)hdr->payload) + (i * max_long_req), size,
            ((char *)dest_ptr) + (i * max_long_req), hdr->args[0], hdr->args[1],
            hdr->args[2], hdr->args[3], hdr->args[4], hdr->args[5], hdr->args[6],
            hdr->args[7], hdr->args[8]));
        break;
      case 10:
        CHECK_GASNET(gasnet_AMRequestLongAsync10(
            peer, hdr->msgid, ((char *)hdr->payload) + (i * max_long_req), size,
            ((char *)dest_ptr) + (i * max_long_req), hdr->args[0], hdr->args[1],
            hdr->args[2], hdr->args[3], hdr->args[4], hdr->args[5], hdr->args[6],
            hdr->args[7], hdr->args[8], hdr->args[9]));
        break;
      case 11:
        CHECK_GASNET(gasnet_AMRequestLongAsync11(
            peer, hdr->msgid, ((char *)hdr->payload) + (i * max_long_req), size,
            ((char *)dest_ptr) + (i * max_long_req), hdr->args[0], hdr->args[1],
            hdr->args[2], hdr->args[3], hdr->args[4], hdr->args[5], hdr->args[6],
            hdr->args[7], hdr->args[8], hdr->args[9], hdr->args[10]));
        break;
      case 12:
        CHECK_GASNET(gasnet_AMRequestLongAsync12(
            peer, hdr->msgid, ((char *)hdr->payload) + (i * max_long_req), size,
            ((char *)dest_ptr) + (i * max_long_req), hdr->args[0], hdr->args[1],
            hdr->args[2], hdr->args[3], hdr->args[4], hdr->args[5], hdr->args[6],
            hdr->args[7], hdr->args[8], hdr->args[9], hdr->args[10], hdr->args[11]));
        break;
      case 13:
        CHECK_GASNET(gasnet_AMRequestLongAsync13(
            peer, hdr->msgid, ((char *)hdr->payload) + (i * max_long_req), size,
            ((char *)dest_ptr) + (i * max_long_req), hdr->args[0], hdr->args[1],
            hdr->args[2], hdr->args[3], hdr->args[4], hdr->args[5], hdr->args[6],
            hdr->args[7], hdr->args[8], hdr->args[9], hdr->args[10], hdr->args[11],
        ...
        break;
      case 14:
        CHECK_GASNET(gasnet_AMRequestLongAsync14(
            peer, hdr->msgid, ((char *)hdr->payload) + (i * max_long_req), size,
            ((char *)dest_ptr) + (i * max_long_req), hdr->args[0], hdr->args[1],
            hdr->args[2], hdr->args[3], hdr->args[4], hdr->args[5], hdr->args[6],
            hdr->args[7], hdr->args[8], hdr->args[9], hdr->args[10], hdr->args[11],
        ...
        break;
      case 15:
        CHECK_GASNET(gasnet_AMRequestLongAsync15(
            peer, hdr->msgid, ((char *)hdr->payload) + (i * max_long_req), size,
            ((char *)dest_ptr) + (i * max_long_req), hdr->args[0], hdr->args[1],
            hdr->args[2], hdr->args[3], hdr->args[4], hdr->args[5], hdr->args[6],
            hdr->args[7], hdr->args[8], hdr->args[9], hdr->args[10], hdr->args[11],
        ...
        break;
      case 16:
        CHECK_GASNET(gasnet_AMRequestLongAsync16(
            peer, hdr->msgid, ((char *)hdr->payload + (i * max_long_req)), size,
            ((char *)dest_ptr) + (i * max_long_req), hdr->args[0], hdr->args[1],
            hdr->args[2], hdr->args[3], hdr->args[4], hdr->args[5], hdr->args[6],
            hdr->args[7], hdr->args[8], hdr->args[9], hdr->args[10], hdr->args[11],
...
void OutgoingMessage::set_payload(PayloadSource *_payload_src, size_t _payload_size,
...
void OutgoingMessage::reserve_srcdata(void)
{
  ...
  if((payload_mode == PAYLOAD_NONE) || (payload_mode == PAYLOAD_EMPTY))
    return;
  ...
  if(need_srcdata) {
    ...
    void *srcptr = 0;
    {
      ...
      if(srcptr != 0) {
        ...
        if(payload_mode == PAYLOAD_COPY) {
          ...
          delete payload_src;
          payload_src = new ContiguousPayload(copy_ptr, payload_size, PAYLOAD_FREE);
    ...
    if(payload_src->get_contig_pointer() && (payload_mode != PAYLOAD_COPY)) {
      payload = payload_src->get_contig_pointer();
...
public:
  EndpointManager(int num_endpoints, int num_dedicated_workers,
                  Realm::CoreReservationSet &crs, bool _use_bgwork)
    ...
    , bgworker_active(_use_bgwork)
  {
    ...
    if(!msgtrace_file) {
      log_amsg.fatal() << "could not open message trace file '" << filename
                       << "': " << strerror(errno);
      abort();
    }
    last_msgtrace_report.store(
        int(Realm::Clock::current_time())); // just keep the integer seconds
#endif
...
public:
  ...
  void handle_channel_flush(gasnet_node_t src, gasnet_handlerarg_t count_lo,
  ...
  bool push_messages(int max_to_send, bool wait, TimeLimit work_until)
  {
    while(true) {
      ...
      bool still_more = endpoints[target]->push_messages(max_to_send, wait, work_until);
  ...
  void report_activemsg_status(FILE *f)
  {
    ...
    for(int i = 0; i < total_endpoints; i++) {
      ...
      fprintf(f, "AMS: %d<->%d: S=%d R=%d\n", mynode, i, e->sent_messages.load(),
              e->received_messages.load());
    }
    fflush(f);
#else
    ...
    for(int i = 0; i < total_endpoints; i++) {
      ...
      fprintf(f, "AMS: %d->%d: S=%zd L=%zd(%zd) W=%d,%d,%zd,%c,%c R=%d,%d\n", mynode, i,
              e->out_short_hdrs.size(), e->out_long_hdrs.size(),
              (e->out_long_hdrs.size() ? (e->out_long_hdrs.front())->payload_size : 0),
...
static void handle_flip_req(gasnet_token_t token, int flip_buffer, int flip_count)
{
  gasnet_node_t src;
...
static void handle_flip_ack(gasnet_token_t token, int ack_buffer)
{
  gasnet_node_t src;
  CHECK_GASNET(gasnet_AMGetMsgSource(token, &src));
...
static void handle_channel_flush(gasnet_token_t token, handlerarg_t count_lo,
                                 handlerarg_t count_hi, handlerarg_t is_quiescent)
{
  gasnet_node_t src;
  CHECK_GASNET(gasnet_AMGetMsgSource(token, &src));
...
static void incoming_message_handled(NodeID sender, uintptr_t comp_info,
                                     uintptr_t msgptr_index)
{
  endpoint_manager->count_handled_message(sender);
  handle_long_msgptr(sender, msgptr_index);
  if(comp_info != 0)
    CHECK_GASNET(gasnet_AMRequestShort1(sender, MSGID_COMPLETION_REPLY, comp_info));
}

static void handle_completion_reply(gasnet_token_t token, gasnet_handlerarg_t arg0)
{
  int index = arg0 >> 2;
...
static void handle_new_activemsg(gasnet_token_t token, void *buf, size_t nbytes,
                                 gasnet_handlerarg_t arg0, gasnet_handlerarg_t arg1,
                                 gasnet_handlerarg_t arg2, gasnet_handlerarg_t arg3,
                                 gasnet_handlerarg_t arg4, gasnet_handlerarg_t arg5,
                                 gasnet_handlerarg_t arg6, gasnet_handlerarg_t arg7,
                                 gasnet_handlerarg_t arg8, gasnet_handlerarg_t arg9,
                                 gasnet_handlerarg_t arg10, gasnet_handlerarg_t arg11,
                                 gasnet_handlerarg_t arg12, gasnet_handlerarg_t arg13,
                                 gasnet_handlerarg_t arg14, gasnet_handlerarg_t arg15)
{
  ...
  bool handle_now =
      ((frag_info == 0) || adjust_long_msgsize(src, buf, nbytes, frag_info));
  if(handle_now) {
...
#ifdef ACTIVE_MESSAGE_TRACE
    log_amsg_trace.info("Active Message Received: %d %d %ld", msg->get_msgid(),
                        msg->get_peer(), msg->get_msgsize());
#endif
#ifdef DEBUG_AMREQUESTS
    ...
    bool handled = incoming_message_manager->add_incoming_message(
        src, msgid, &header.args[6], 10 * 4, PAYLOAD_COPY, buf, nbytes, PAYLOAD_KEEP,
        incoming_message_handled, deferred_comp_info, msgptr_index,
        ((ThreadLocal::gasnet_work_until != 0) ? *ThreadLocal::gasnet_work_until
                                               : TimeLimit()));
    if(handled) {
      ...
      if(comp_info != 0)
        CHECK_GASNET(gasnet_AMReplyShort1(token, MSGID_COMPLETION_REPLY, comp_info));
    } else {
      if((comp_info & PendingCompletion::LOCAL_PENDING_BIT) != 0) {
        gasnet_handlerarg_t local_comp_only =
            (comp_info & ~PendingCompletion::REMOTE_PENDING_BIT);
        CHECK_GASNET(
            gasnet_AMReplyShort1(token, MSGID_COMPLETION_REPLY, local_comp_only));
      }
    }
  } else
...
void init_endpoints(size_t gasnet_mem_size, size_t registered_mem_size,
                    size_t registered_ib_mem_size, CoreReservationSet &crs,
                    int num_worker_threads, BackgroundWorkManager &bgwork,
                    bool poll_use_bgwork, IncomingMessageManager *message_manager)
{
  size_t total_lmb_size = (gasnet_nodes() * num_lmbs * lmb_size);

  // add in our internal handlers and space we need for LMBs
  size_t attach_size = (gasnet_mem_size + registered_mem_size + registered_ib_mem_size +
                        srcdatapool_size + total_lmb_size);

  if(gasnet_mynode() == 0) {
    log_amsg.info() << "Pinned Memory Usage: GASNET=" << (gasnet_mem_size << 20)
                    << " MB, RMEM=" << (registered_mem_size >> 20)
                    << " MB, IBRMEM=" << (registered_ib_mem_size >> 20)
                    << " MB, LMB=" << (total_lmb_size >> 20)
                    << " MB, SDP=" << (srcdatapool_size >> 20)
                    << " MB, total=" << (attach_size >> 20) << " MB";
  ...
  CHECK_GASNET(gasnet_attach(handlers, hcount, attach_size, 0));
  ...
  char *srcdatapool_base = my_segment;
  my_segment += srcdatapool_size;
  /*char *lmb_base = my_segment;*/ my_segment += total_lmb_size;
  assert(my_segment <= ((char *)(segment_info[gasnet_mynode()].addr) +
                        segment_info[gasnet_mynode()].size));
  ...
  endpoint_manager =
      new EndpointManager(gasnet_nodes(), num_worker_threads, crs, poll_use_bgwork);
  if(poll_use_bgwork)
...
void EndpointManager::start_polling_threads(void)
{
  ...
  for(int i = 0; i < dedicated_workers; i++)
    polling_threads[i] =
        Realm::Thread::create_kernel_thread<EndpointManager,
                                            &EndpointManager::polling_worker_loop>(
            this, Realm::ThreadLaunchParameters(), *core_rsrv);
...
{
  // none of our threads actually sleep, so we can just set the flag and wait for them to
  // notice
  shutdown_flag = true;

  for(std::vector<Realm::Thread *>::iterator it = polling_threads.begin();
      it != polling_threads.end(); it++) {
    (*it)->join();
    delete(*it);
  }
...
void EndpointManager::polling_worker_loop(void)
{
  while(true) {
    bool still_more =
        endpoint_manager->push_messages(max_msgs_to_send, false /*!wait*/, TimeLimit());
...
void start_polling_threads(void) { endpoint_manager->start_polling_threads(); }
...
void enqueue_message(NodeID target, int msgid, const void *args, size_t arg_size,
                     const void *payload, size_t payload_size, int payload_mode,
                     PendingCompletion *comp, void *dstptr)
{
  assert((gasnet_node_t)target != gasnet_mynode());

  OutgoingMessage *hdr =
      new OutgoingMessage(msgid, (arg_size + sizeof(int) - 1) / sizeof(int), args, comp);
...
void enqueue_message(NodeID target, int msgid, const void *args, size_t arg_size,
                     const void *payload, size_t line_size, off_t line_stride,
                     size_t line_count, int payload_mode, PendingCompletion *comp,
                     void *dstptr)
{
  assert((gasnet_node_t)target != gasnet_mynode());
  ...
  if(payload_mode != PAYLOAD_NONE) {
    ...
    if(payload_size > 0) {
      PayloadSource *payload_src =
          new TwoDPayload(payload, line_size, line_count, line_stride, payload_mode);
      hdr->set_payload(payload_src, payload_size, payload_mode, dstptr);
    } else {
      hdr->set_payload_empty();
    }
...
void enqueue_message(NodeID target, int msgid, const void *args, size_t arg_size,
                     const SpanList &spans, size_t payload_size, int payload_mode,
                     PendingCompletion *comp, void *dstptr)
{
  assert((gasnet_node_t)target != gasnet_mynode());
                                ...
                                int frag_info)
{
  ...
  return endpoint_manager->adjust_long_msgsize(source, ptr, buffer_size, frag_info);
}
...
bool QuiescenceChecker::perform_check(size_t sampled_receive_count)
{
  ...
  log_amsg.info() << "local quiescence: prev=" << last_message_count
                  << " cur=" << total_sent << " new_rcvd=" << new_messages_rcvd;
...
TwoDPayload::TwoDPayload(const void *_srcptr, size_t _line_size, size_t _line_count,
                         ptrdiff_t _line_stride, int _mode)
  : srcptr(_srcptr)
  , line_size(_line_size)
  , line_count(_line_count)
  , line_stride(_line_stride)
  , mode(_mode)
{}
...
PendingCompletion *PendingCompletionManager::get_available()
{
  ...
  for(size_t i = 0; i < (1 << PendingCompletionGroup::LOG2_GROUPSIZE); i++)
    newgrp->entries[i].index =
        ((grp_index << PendingCompletionGroup::LOG2_GROUPSIZE) + i);
  ...
  PendingCompletion **new_tail =
      &newgrp->entries[(1 << PendingCompletionGroup::LOG2_GROUPSIZE) - 1].next_free;
